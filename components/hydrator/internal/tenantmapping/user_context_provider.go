...
func (m *userContextProvider) GetObjectContext(ctx context.Context, reqData oathkeeper.ReqData, authDetails oathkeeper.AuthDetails) (ObjectContext, error) {
	...
	if err != nil {
		...
		return NewObjectContext(&graphql.Tenant{}, m.tenantKeys, scopes, intersectWithOtherScopes, authDetails.Region, "", authDetails.AuthID, authDetails.AuthFlow, consumer.User, tenantmapping.UserObjectContextProvider, authDetails.Subject), nil
	}
	...
	if err != nil {
		if directorErrors.IsGQLNotFoundError(err) {
			...
			return NewObjectContext(&graphql.Tenant{ID: externalTenantID}, m.tenantKeys, scopes, intersectWithOtherScopes, "", "", authDetails.AuthID, authDetails.AuthFlow, consumer.User, tenantmapping.UserObjectContextProvider, authDetails.Subject), nil
		}
	...
	objCtx := NewObjectContext(tenantMapping, m.tenantKeys, scopes, intersectWithOtherScopes, authDetails.Region, "", authDetails.AuthID, authDetails.AuthFlow, consumer.User, tenantmapping.UserObjectContextProvider, authDetails.Subject)
