...
func (svc *certificateService) SignCSR(ctx context.Context, encodedCSR []byte, subject CSRSubject) (EncodedCertificateChain, apperrors.AppError) {
	...
	if err != nil {
		log.C(ctx).Errorf("Error occurred while loading the CSR with Common Name %s", subject.CommonName)
	...
	if err != nil {
		log.C(ctx).Errorf("Error occurred while checking the values of the CSR with Common Name %s", subject.CommonName)
		return EncodedCertificateChain{}, err
	}
