...
type config struct {
	...
	RootAPI string `envconfig:"APP_ROOT_API,default=/tenantsondemand"`
...
type securityConfig struct {
	...
	TenantOnDemandCallbackScope string        `envconfig:"APP_TENANT_ON_DEMAND_CALLBACK_SCOPE"`
}
...
func configureAuthMiddleware(ctx context.Context, httpClient *http.Client, router *mux.Router, cfg securityConfig) {
	scopeValidator := claims.NewScopesValidator([]string{cfg.TenantOnDemandCallbackScope})
...
func registerHandler(ctx context.Context, router *mux.Router, cfg tenantfetcher.HandlerConfig) {
	...
	fetcher := tenantfetcher.NewFetcher(directorClient, provisioner)
