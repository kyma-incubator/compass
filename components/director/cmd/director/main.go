...
func main() {
	...
	gqlCfg := graphql.Config{
		...
		Directives: graphql.DirectiveRoot{
			...
			HasScenario: scenario.NewDirective(transact, label.NewScenarioService(label.NewRepository(label.NewConverter())), bundleRepo(), bundleInstanceAuthRepo()).HasScenario,
...
func getRuntimeMappingHandlerFunc(transact persistence.Transactioner, cachePeriod time.Duration, ctx context.Context, defaultScenarioEnabled bool, protectedLabelPattern string, httpClient *http.Client) (func(writer http.ResponseWriter, request *http.Request), error) {
	...
	scenarioAssignmentEngine := scenarioassignment.NewEngine(labelUpsertSvc, labelRepo, scenarioAssignmentRepo, bundleInstanceAuthSvc, runtimeRepo)

	runtimeSvc := runtime.NewService(runtimeRepo, labelRepo, scenariosDefSvc, scenariosSvc, labelUpsertSvc, uidSvc, scenarioAssignmentEngine, bundleInstanceAuthSvc, protectedLabelPattern)
