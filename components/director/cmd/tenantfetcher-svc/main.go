...
import (
	...
	tf "github.com/kyma-incubator/compass/components/director/internal/tenantfetchersvc"
...
type config struct {
	...
	Handler   tf.HandlerConfig
	EventsCfg tf.EventsConfig
...
func readJobConfig(ctx context.Context, jobName string, environmentVars map[string]string) tf.JobConfig {
	return tf.NewTenantFetcherJobEnvironment(ctx, jobName, environmentVars).ReadJobConfig()
}

func runTenantFetcherJob(ctx context.Context, jobConfig tf.JobConfig, metricsReporter metrics.MetricsReporter, stopJob chan bool) func() error {
...
func syncTenants(ctx context.Context, jobConfig tf.JobConfig, metricsReporter metrics.MetricsReporter, transact persistence.Transactioner) {
...
func createMetricsReporter(ctx context.Context, jobConfig tf.JobConfig) metrics.MetricsReporter {
...
func createTenantsFetcherSvc(ctx context.Context, jobConfig tf.JobConfig, transact persistence.Transactioner) (tf.TenantSyncService, error) {
...
func registerTenantsHandler(ctx context.Context, router *mux.Router, cfg tf.HandlerConfig) {
	gqlClient := newInternalGraphQLClient(cfg.DirectorGraphQLEndpoint, cfg.ClientTimeout, cfg.HTTPClientSkipSslValidation)
	directorClient := graphqlclient.NewDirector(gqlClient)

	tenantConverter := tenant.NewConverter()

	provisioner := tf.NewTenantProvisioner(directorClient, tenantConverter, cfg.TenantProvider)
	subscriber := tf.NewSubscriber(directorClient, provisioner)
	tenantHandler := tf.NewTenantsHTTPHandler(subscriber, cfg)
...
func registerTenantsOnDemandHandler(ctx context.Context, router *mux.Router, eventsCfg tf.EventsConfig, tenantHandlerCfg tf.HandlerConfig, transact persistence.Transactioner) {
	...
	fetcher := tf.NewTenantFetcher(*onDemandSvc)
	tenantHandler := tf.NewTenantFetcherHTTPHandler(fetcher, tenantHandlerCfg)
...
func createTenantFetcherOnDemandSvc(eventsCfg tf.EventsConfig, handlerCfg tf.HandlerConfig, transact persistence.Transactioner) (*tf.SubaccountOnDemandService, error) {
	eventAPIClient, err := tf.NewClient(eventsCfg.OAuthConfig, eventsCfg.AuthMode, eventsCfg.APIConfig, handlerCfg.ClientTimeout)
	if nil != err {
		return nil, err
	}
