package cert_test

import (
	"encoding/base64"
	"testing"

	"github.com/kyma-incubator/compass/components/director/pkg/cert"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func TestSubjectExtraction(t *testing.T) {
	for _, testCase := range []struct {
		subject          string
		orgUnitPattern   string
		orgRegionPattern string
		country          string
		locality         string
		province         string
		org              string
		orgUnit          string
		orgUnits         []string
		uuidOrgUnit      string
		remainingOrgUnit string
		commonName       string
	}{
		{
			subject:          "CN=application,OU=SAP Cloud Platform Clients,OU=OrgUnit2,OU=123e4567-e89b-12d3-a456-426614174001,O=Org,L=Waldorf,ST=Waldorf,C=DE",
			orgUnitPattern:   "SAP Cloud Platform Clients",
			orgRegionPattern: "OrgUnit2",
			country:          "DE",
			locality:         "Waldorf",
			province:         "Waldorf",
			org:              "Org",
			orgUnit:          "SAP Cloud Platform Clients",
			orgUnits:         []string{"SAP Cloud Platform Clients", "OrgUnit2", "123e4567-e89b-12d3-a456-426614174001"},
			uuidOrgUnit:      "123e4567-e89b-12d3-a456-426614174001",
			remainingOrgUnit: "123e4567-e89b-12d3-a456-426614174001",
			commonName:       "application",
		},
		{
			subject:          "CN=application,OU=SAP Cloud Platform Clients,OU=123e4567-e89b-12d3-a456-426614174001,O=Org,L=Waldorf,ST=Waldorf,C=DE",
			orgUnitPattern:   "SAP Cloud Platform Clients",
			country:          "DE",
			locality:         "Waldorf",
			province:         "Waldorf",
			org:              "Org",
			orgUnit:          "SAP Cloud Platform Clients",
			orgUnits:         []string{"SAP Cloud Platform Clients", "123e4567-e89b-12d3-a456-426614174001"},
			uuidOrgUnit:      "123e4567-e89b-12d3-a456-426614174001",
			remainingOrgUnit: "123e4567-e89b-12d3-a456-426614174001",
			commonName:       "application",
		},
		{
			subject:          "CN=application,OU=SAP Cloud Platform Clients,OU=123e4567-e89b-12d3-a456-426614174001,O=Org,L=Waldorf,ST=Waldorf,C=DE",
			orgUnitPattern:   "SAP Cloud Platform Clients",
			orgRegionPattern: "OrgUnit2",
			country:          "DE",
			locality:         "Waldorf",
			province:         "Waldorf",
			org:              "Org",
			orgUnit:          "SAP Cloud Platform Clients",
			orgUnits:         []string{"SAP Cloud Platform Clients", "123e4567-e89b-12d3-a456-426614174001"},
			uuidOrgUnit:      "123e4567-e89b-12d3-a456-426614174001",
			remainingOrgUnit: "123e4567-e89b-12d3-a456-426614174001",
			commonName:       "application",
		},
		{
			subject:          "CN=application,OU=SAP Cloud Platform Clients,OU=OrgUnit2,OU=RemainingOrgUnit,O=Org,L=Waldorf,ST=Waldorf,C=DE",
			orgUnitPattern:   "SAP Cloud Platform Clients",
			orgRegionPattern: "OrgUnit2",
			country:          "DE",
			locality:         "Waldorf",
			province:         "Waldorf",
			org:              "Org",
			orgUnit:          "SAP Cloud Platform Clients",
			orgUnits:         []string{"SAP Cloud Platform Clients", "OrgUnit2", "RemainingOrgUnit"},
			uuidOrgUnit:      "",
			remainingOrgUnit: "RemainingOrgUnit",
			commonName:       "application",
		},
		{
			subject:          "CN=application,OU=SAP Cloud Platform Clients,OU=OrgUnit2,OU=RemainingOrgUnit,OU=RemainingOrgUnit2,O=Org,L=Waldorf,ST=Waldorf,C=DE",
			orgUnitPattern:   "SAP Cloud Platform Clients",
			orgRegionPattern: "OrgUnit2",
			country:          "DE",
			locality:         "Waldorf",
			province:         "Waldorf",
			org:              "Org",
			orgUnit:          "SAP Cloud Platform Clients",
			orgUnits:         []string{"SAP Cloud Platform Clients", "OrgUnit2", "RemainingOrgUnit", "RemainingOrgUnit2"},
			uuidOrgUnit:      "",
			remainingOrgUnit: "",
			commonName:       "application",
		},
		{
			subject:        "CN=,OU=,O=,L=,ST=,C=",
			orgUnitPattern: "(SAP Cloud Platform Clients|OrgUnit2|OrgUnit3)|OrgUnit4",
			country:        "",
			locality:       "",
			province:       "",
			org:            "",
			orgUnit:        "",
			orgUnits:       []string{},
			commonName:     "",
		},
		{
			subject:    "CN=,OU=,O=,L=,ST=,C=",
			country:    "",
			locality:   "",
			province:   "",
			org:        "",
			orgUnit:    "",
			orgUnits:   []string{},
			commonName: "",
		},
	} {
		t.Run("should extract subject values", func(t *testing.T) {
			assert.Equal(t, testCase.country, cert.GetCountry(testCase.subject))
			assert.Equal(t, testCase.locality, cert.GetLocality(testCase.subject))
			assert.Equal(t, testCase.province, cert.GetProvince(testCase.subject))
			assert.Equal(t, testCase.org, cert.GetOrganization(testCase.subject))
			assert.Equal(t, testCase.orgUnit, cert.GetOrganizationalUnit(testCase.subject))
			assert.Equal(t, testCase.orgUnits, cert.GetAllOrganizationalUnits(testCase.subject))
			assert.Equal(t, testCase.uuidOrgUnit, cert.GetUUIDOrganizationalUnit(testCase.subject))
			assert.Equal(t, testCase.remainingOrgUnit, cert.GetRemainingOrganizationalUnit(testCase.orgUnitPattern, testCase.orgRegionPattern)(testCase.subject))
			assert.Equal(t, testCase.commonName, cert.GetCommonName(testCase.subject))
		})
	}
}

func TestParseCertificate(t *testing.T) {
	const certificate = "-----BEGIN CERTIFICATE-----\\nL8uAkeTGuHEATjmosQWUmbYi\n-----END CERTIFICATE-----\n"
	const key = "-----BEGIN RSA PRIVATE KEY-----\\nGb5Nk5evWBGhsl2LFqoOqhvFpjftv8+qgRHxmWtj4EoJYWng+hRz\n-----END RSA PRIVATE KEY-----\n"

	t.Run("non-base64-encoded-cert", func(t *testing.T) {
		_, err := cert.ParseCertificate(certificate, key)
		require.NoError(t, err, "failed to parse certificate")
	})

	t.Run("base64-encoded-cert", func(t *testing.T) {
		base64EncodedCert := base64.StdEncoding.EncodeToString([]byte(certificate))
		base64EncodedKey := base64.StdEncoding.EncodeToString([]byte(key))

		_, err := cert.ParseCertificate(base64EncodedCert, base64EncodedKey)
		require.NoError(t, err, "failed to parse certificate")
	})
}
