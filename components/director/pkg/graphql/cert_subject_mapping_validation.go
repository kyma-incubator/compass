...
// Validate validates the CertificateSubjectMappingInput structure values
func (i CertificateSubjectMappingInput) Validate() error {
	return validation.ValidateStruct(&i,
		...
		validation.Field(&i.ConsumerType, validation.Required, validation.Length(1, 256)),
		validation.Field(&i.InternalConsumerID, is.UUID),
		validation.Field(&i.TenantAccessLevels, validation.Required),
	)
}
