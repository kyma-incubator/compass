...
func TestX509Config_ParseCertificateCertificate(t *testing.T) {
	...
	base64EncodedCert := base64.StdEncoding.EncodeToString([]byte(cert))
	base64EncodedKey := base64.StdEncoding.EncodeToString([]byte(key))

	config := X509Config{Cert: base64EncodedCert, Key: base64EncodedKey}
	_, err := config.ParseCertificate()
