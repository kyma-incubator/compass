...
func (p *certServiceContextProvider) verifyTenantAccess(ctx context.Context, tenant *model.BusinessTenantMapping, authDetails oathkeeper.AuthDetails, reqData oathkeeper.ReqData) error {
	...
	if data.AccessLevel != "" && tenant.Type != data.AccessLevel {
		return apperrors.NewUnauthorizedError(fmt.Sprintf("Certificate with auth ID %s has no access to %s with ID %s", authDetails.AuthID, data.AccessLevel, tenant.ExternalTenant))
	}
