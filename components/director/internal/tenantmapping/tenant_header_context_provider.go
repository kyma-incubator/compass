...
func (p *tenantHeaderContextProvider) GetObjectContext(ctx context.Context, reqData oathkeeper.ReqData, authDetails oathkeeper.AuthDetails) (ObjectContext, error) {
	...
	if err != nil {
		if apperrors.IsNotFoundError(err) {
			...
			return NewObjectContext(NewTenantContext(externalTenantID, ""), p.tenantKeys, "", authDetails.Region, "", authDetails.AuthID, authDetails.AuthFlow, consumer.ConsumerType(consumerType), CertServiceObjectContextProvider), nil
		}
	...
	objCtx := NewObjectContext(NewTenantContext(externalTenantID, tenantMapping.ID), p.tenantKeys, "",
