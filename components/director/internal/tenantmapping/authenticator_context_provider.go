...
func (m *authenticatorContextProvider) GetObjectContext(ctx context.Context, reqData oathkeeper2.ReqData, authDetails oathkeeper2.AuthDetails) (ObjectContext, error) {
	...
	if err != nil {
		if apperrors.IsNotFoundError(err) {
			...
			return NewObjectContext(NewTenantContext(externalTenantID, ""), m.tenantKeys, scopes, intersectWithOtherScopes, authDetails.Region, clientID, authDetails.AuthID, authDetails.AuthFlow, consumer.User, AuthenticatorObjectContextProvider), nil
		}
