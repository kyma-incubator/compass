// Code generated by mockery v1.0.0. DO NOT EDIT.

package automock

import context "context"
import mock "github.com/stretchr/testify/mock"
import tenantmapping "github.com/kyma-incubator/compass/components/director/internal/tenantmapping"

// ObjectContextForSystemAuthProvider is an autogenerated mock type for the ObjectContextForSystemAuthProvider type
type ObjectContextForSystemAuthProvider struct {
	mock.Mock
}

// GetObjectContext provides a mock function with given fields: ctx, reqData, authID, authFlow
func (_m *ObjectContextForSystemAuthProvider) GetObjectContext(ctx context.Context, reqData tenantmapping.ReqData, authID string, authFlow tenantmapping.AuthFlow) (tenantmapping.ObjectContext, error) {
	ret := _m.Called(ctx, reqData, authID, authFlow)

	var r0 tenantmapping.ObjectContext
	if rf, ok := ret.Get(0).(func(context.Context, tenantmapping.ReqData, string, tenantmapping.AuthFlow) tenantmapping.ObjectContext); ok {
		r0 = rf(ctx, reqData, authID, authFlow)
	} else {
		r0 = ret.Get(0).(tenantmapping.ObjectContext)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, tenantmapping.ReqData, string, tenantmapping.AuthFlow) error); ok {
		r1 = rf(ctx, reqData, authID, authFlow)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}
