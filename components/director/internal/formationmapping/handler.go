...
type formationAssignmentStatusService interface {
	...
	SetAssignmentToErrorStateWithConstraints(ctx context.Context, notificationResponse *statusreport.NotificationStatusReport, assignment *model.FormationAssignment, errorMessage string, errorCode formationassignment.AssignmentErrorCode, state model.FormationAssignmentState, operation model.FormationOperation) error
...
func (h *Handler) processFormationAssignmentUnassignStatusUpdate(ctx context.Context, fa *model.FormationAssignment, reqBody FormationAssignmentRequestBody) (bool, error) {
	...
	statusReport := newNotificationStatusReportFromRequestBody(reqBody)

	if reqBody.State == model.DeleteErrorAssignmentState {
		if err := h.faStatusService.SetAssignmentToErrorStateWithConstraints(ctx, statusReport, fa, reqBody.Error, formationassignment.ClientError, reqBody.State, model.UnassignFormation); err != nil {
...
func (h *Handler) processFormationAssignmentAssignStatusUpdate(ctx context.Context, fa *model.FormationAssignment, reqBody FormationAssignmentRequestBody, correlationID string) (bool, *responseError) {
	...
	if reqBody.State == model.CreateErrorAssignmentState {
		if err := h.faStatusService.SetAssignmentToErrorStateWithConstraints(ctx, statusReport, fa, reqBody.Error, formationassignment.ClientError, reqBody.State, model.AssignFormation); err != nil {
