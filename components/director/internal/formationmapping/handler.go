...
func (h *Handler) UpdateFormationAssignmentStatus(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.C(ctx).WithError(err).Errorf("An error occurred while updating formation assignment with ID: %q and formation ID: %q to: %q state", formationAssignmentID, formationID, reqBody.State)
		respondWithError(ctx, w, http.StatusInternalServerError, errResp)
		return
	}
