...
func TestUpdate_Handler(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			Request:              createRequestWithClaims(testID, consumer.Application, oathkeeper.CertificateFlow),
			ExpectedStatus:       http.StatusOK,
			ExpectedErrorMessage: str.Ptr("An error has occurred while checking repository status for applications with ID foo:"),
			ExpectedError:        str.Ptr("test"),
			MockNextHandler:      fixNextHandler(t),
		},
		{
		...
		},
		{
			...
			Request:              createRequestWithClaims(testID, consumer.Application, oathkeeper.CertificateFlow),
			ExpectedStatus:       http.StatusOK,
			ExpectedErrorMessage: str.Ptr("An error has occurred while updating repository status for applications with ID foo:"),
			ExpectedError:        str.Ptr("test"),
			MockNextHandler:      fixNextHandler(t),
		},
	}
