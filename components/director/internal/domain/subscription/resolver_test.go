...
func TestResolver_SubscribeTenant(t *testing.T) {
	...
	providerSubaccountID := "provider-subaccount"
	...
	testCases := []struct {
		...
		},
		{
			...
			ServiceFn: func() *automock.SubscriptionService {
				...
				svc.On("SubscribeTenantToRuntime", txtest.CtxWithDBMatcher(), subscriptionProviderID, subscribedSubaacountID, providerSubaccountID, consumerTenantID, region, subscriptionAppName).Return(true, nil).Once()
				return svc
			},
			ExpectedOutput: true,
			ExpectedErr:    nil,
		...
		},
		{
			...
			ServiceFn: func() *automock.SubscriptionService {
				svc := &automock.SubscriptionService{}
				svc.On("DetermineSubscriptionFlow", txtest.CtxWithDBMatcher(), subscriptionProviderID, region).Return(resource.Runtime, nil).Once()
				svc.On("SubscribeTenantToRuntime", txtest.CtxWithDBMatcher(), subscriptionProviderID, subscribedSubaacountID, providerSubaccountID, consumerTenantID, region, subscriptionAppName).Return(false, testErr).Once()
				svc.AssertNotCalled(t, "SubscribeTenantToApplication")
				return svc
			},
			ExpectedOutput: false,
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			// WHEN
			result, err := resolver.SubscribeTenant(context.TODO(), subscriptionProviderID, subscribedSubaacountID, providerSubaccountID, consumerTenantID, region, subscriptionAppName)
...
func TestResolver_UnsubscribeTenant(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			ServiceFn: func() *automock.SubscriptionService {
				svc := &automock.SubscriptionService{}
				svc.On("UnsubscribeTenantFromRuntime", txtest.CtxWithDBMatcher(), subscriptionProviderID, subscribedSubaacountID, providerSubaccountID, consumerTenantID, region).Return(true, nil).Once()
				svc.On("DetermineSubscriptionFlow", txtest.CtxWithDBMatcher(), subscriptionProviderID, region).Return(resource.Runtime, nil).Once()
		...
		},
		{
			...
			ServiceFn: func() *automock.SubscriptionService {
				svc := &automock.SubscriptionService{}
				svc.On("UnsubscribeTenantFromRuntime", txtest.CtxWithDBMatcher(), subscriptionProviderID, subscribedSubaacountID, providerSubaccountID, consumerTenantID, region).Return(false, testErr).Once()
				svc.On("DetermineSubscriptionFlow", txtest.CtxWithDBMatcher(), subscriptionProviderID, region).Return(resource.Runtime, nil).Once()
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			// WHEN
			result, err := resolver.UnsubscribeTenant(context.TODO(), subscriptionProviderID, subscribedSubaacountID, providerSubaccountID, consumerTenantID, region)
