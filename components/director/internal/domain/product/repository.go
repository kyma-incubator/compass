...
const packageTable string = `public.products`

var (
	tenantColumn   = "tenant_id"
	packageColumns = []string{"ord_id", tenantColumn, "app_id", "title", "short_description", "vendor", "parent", "sap_ppms_object_id", "labels"}
...
func NewRepository(conv EntityConverter) *pgRepository {
	return &pgRepository{
		conv:         conv,
		existQuerier: repo.NewExistQuerier(resource.Product, packageTable, tenantColumn),
		singleGetter: repo.NewSingleGetter(resource.Product, packageTable, tenantColumn, packageColumns),
		deleter:      repo.NewDeleter(resource.Product, packageTable, tenantColumn),
		creator:      repo.NewCreator(resource.Product, packageTable, packageColumns),
		updater:      repo.NewUpdater(resource.Product, packageTable, updatableColumns, tenantColumn, []string{"ord_id"}),
	}
}
