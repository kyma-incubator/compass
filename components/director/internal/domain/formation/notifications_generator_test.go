...
func Test_GenerateFormationLifecycleNotifications(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			notificationsBuilderFn: func() *automock.NotificationBuilder {
				...
				notificationBuilder.On("BuildFormationNotificationRequests", ctx, formationNotificationDetails, formationInput, formationLifecycleWebhooksSync).Return(formationNotificationRequests, nil).Once()
		...
		},
		{
			...
			notificationsBuilderFn: func() *automock.NotificationBuilder {
				notificationBuilder := &automock.NotificationBuilder{}
				notificationBuilder.On("BuildFormationNotificationRequests", ctx, formationNotificationDetails, formationInput, formationLifecycleWebhooksSync).Return(nil, testErr).Once()
				return notificationBuilder
			},
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			formationNotificationReqs, err := notificationGenerator.GenerateFormationLifecycleNotifications(ctx, formationLifecycleWebhooksSync, TntInternalID, formationInput, testFormationTemplateName, FormationTemplateID, model.CreateFormation, CustomerTenantContextAccount)
