...
func Test_NotificationService_GenerateFormationNotifications(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			notificationsGeneratorFn: func() *automock.NotificationsGenerator {
				...
				notificationGenerator.On("GenerateFormationLifecycleNotifications", ctx, formationLifecycleWebhooksSync, TntInternalID, formationInput, testFormationTemplateName, FormationTemplateID, model.CreateFormation, CustomerTenantContextAccount).Return(formationNotificationRequests, nil).Once()
		...
		},
		{
			...
			notificationsGeneratorFn: func() *automock.NotificationsGenerator {
				notificationGenerator := &automock.NotificationsGenerator{}
				notificationGenerator.On("GenerateFormationLifecycleNotifications", ctx, formationLifecycleWebhooksSync, TntInternalID, formationInput, testFormationTemplateName, FormationTemplateID, model.CreateFormation, CustomerTenantContextAccount).Return(nil, testErr).Once()
				return notificationGenerator
			},
			expectedErrMsg: testErr.Error(),
		},
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			formationNotificationReqs, err := notificationSvc.GenerateFormationNotifications(ctx, formationLifecycleWebhooksSync, TntInternalID, formationInput, testFormationTemplateName, FormationTemplateID, model.CreateFormation)
