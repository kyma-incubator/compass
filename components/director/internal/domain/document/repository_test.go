...
func TestRepository_CreateMany(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		...
		given := []*model.Document{
			fixModelDocument("1", bundleID()),
			fixModelDocument("2", bundleID()),
			fixModelDocument("3", bundleID()),
		}
		expected := []*document.Entity{
			fixEntityDocument("1", bundleID()),
			fixEntityDocument("2", bundleID()),
			fixEntityDocument("3", bundleID()),
		}
		...
		for i, givenModel := range given {
			...
			dbMock.ExpectExec(regexp.QuoteMeta(insertQuery)).WithArgs(
				expectedEntity.ID, expectedEntity.TenantID, expectedEntity.BundleID, expectedEntity.Title, expectedEntity.DisplayName, expectedEntity.Description, expectedEntity.Format, expectedEntity.Kind, expectedEntity.Data).WillReturnResult(sqlmock.NewResult(-1, 1))
		}
	...
	t.Run("DB Error", func(t *testing.T) {
		...
		dbMock.ExpectExec(regexp.QuoteMeta(insertQuery)).WithArgs(
			expected[0].ID, expected[0].TenantID, expected[0].BundleID, expected[0].Title, expected[0].DisplayName, expected[0].Description, expected[0].Format, expected[0].Kind, expected[0].Data).WillReturnResult(sqlmock.NewResult(-1, 1))
		dbMock.ExpectExec(regexp.QuoteMeta(insertQuery)).WithArgs(
			expected[1].ID, expected[1].TenantID, expected[1].BundleID, expected[1].Title, expected[1].DisplayName, expected[1].Description, expected[1].Format, expected[1].Kind, expected[1].Data).WillReturnError(givenError())
...
func TestRepository_ListForBundle(t *testing.T) {
	...
	inputCursor := ""
	totalCount := 2
	docEntity1 := fixEntityDocument("1", bundleID())
	docEntity2 := fixEntityDocument("2", bundleID())
	...
	t.Run("Success", func(t *testing.T) {
		rows := sqlmock.NewRows(columns).
			AddRow(docEntity1.ID, docEntity1.TenantID, docEntity1.BundleID, docEntity1.Title, docEntity1.DisplayName, docEntity1.Description, docEntity1.Format, docEntity1.Kind, docEntity1.Data).
			AddRow(docEntity2.ID, docEntity2.TenantID, docEntity2.BundleID, docEntity2.Title, docEntity2.DisplayName, docEntity2.Description, docEntity2.Format, docEntity2.Kind, docEntity2.Data)
	...
	t.Run("DB Error", func(t *testing.T) {
		...
		pgRepository := document.NewRepository(conv)
		// WHEN
		_, err := pgRepository.ListForBundle(ctx, tenantID, bundleID(), 3, "")
		//THEN
		require.Error(t, err)
	...
	t.Run("Converter Error", func(t *testing.T) {
		...
		repo := document.NewRepository(conv)
		//WHEN
		_, err := repo.ListForBundle(ctx, tenantID, bundleID(), inputPageSize, inputCursor)
		//THEN
		require.Error(t, err)
		require.Contains(t, err.Error(), testErr.Error())
	})
...
func TestRepository_GetForBundle(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		dbMock.ExpectQuery(regexp.QuoteMeta(query)).
			WithArgs(givenTenant(), givenID(), bundleID()).WillReturnRows(rows)

		ctx := persistence.SaveToContext(context.TODO(), db)
		// WHEN
		actual, err := repo.GetForBundle(ctx, givenTenant(), givenID(), bundleID())
		// THEN
		require.NoError(t, err)
		require.NotNil(t, actual)
		assert.Equal(t, docModel, actual)
	...
	t.Run("Converter Error", func(t *testing.T) {
		...
		ctx := persistence.SaveToContext(context.TODO(), db)
		// WHEN
		_, err := repo.GetForBundle(ctx, givenTenant(), givenID(), bundleID())
		// THEN
		require.EqualError(t, err, "while converting Document entity to model: some error")
	})
	...
	t.Run("DB Error", func(t *testing.T) {
		...
		dbMock.ExpectQuery("SELECT .*").
			WithArgs(givenTenant(), givenID(), bundleID()).WillReturnError(givenError())
