...
var columns = []string{"id", "bundle_id", "app_id", "app_template_version_id", "title", "display_name", "description", "format", "kind", "data", "ready", "created_at", "updated_at", "deleted_at", "error"}

func TestRepository_Create(t *testing.T) {
	refID := bndlID()
	var nilDocModel *model.Document
	docModel := fixModelDocumentForApp(givenID(), refID)
	docEntity := fixEntityDocumentForApp(givenID(), refID)
	...
	suite := testdb.RepoCreateTestSuite{
		...
		SQLQueryDetails: []testdb.SQLQueryDetails{
			...
			{
				Query: regexp.QuoteMeta("INSERT INTO public.documents ( id, bundle_id, app_id, app_template_version_id, title, display_name, description, format, kind, data, ready, created_at, updated_at, deleted_at, error ) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )"),
				Args: []driver.Value{givenID(), refID, appID, repo.NewValidNullableString(""), docEntity.Title, docEntity.DisplayName, docEntity.Description, docEntity.Format, docEntity.Kind, docEntity.Data,
...
func TestRepository_CreateGlobal(t *testing.T) {
	refID := bndlID()
	var nilDocModel *model.Document
	docModel := fixModelDocumentForAppTemplateVersion(givenID(), refID)
	docEntity := fixEntityDocumentForAppTemplateVersion(givenID(), refID)

	suite := testdb.RepoCreateTestSuite{
		Name: "Create Document Global",
		SQLQueryDetails: []testdb.SQLQueryDetails{
			{
				Query: regexp.QuoteMeta("INSERT INTO public.documents ( id, bundle_id, app_id, app_template_version_id, title, display_name, description, format, kind, data, ready, created_at, updated_at, deleted_at, error ) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )"),
				Args: []driver.Value{givenID(), refID, repo.NewValidNullableString(""), appTemplateVersionID, docEntity.Title, docEntity.DisplayName, docEntity.Description, docEntity.Format, docEntity.Kind, docEntity.Data,
					docEntity.Ready, docEntity.CreatedAt, docEntity.UpdatedAt, docEntity.DeletedAt, docEntity.Error},
				ValidResult: sqlmock.NewResult(-1, 1),
			},
		},
...
func TestRepository_CreateMany(t *testing.T) {
	...
	insertQuery := regexp.QuoteMeta("INSERT INTO public.documents ( id, bundle_id, app_id, app_template_version_id, title, display_name, description, format, kind, data, ready, created_at, updated_at, deleted_at, error ) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )")
	...
	t.Run("Success", func(t *testing.T) {
		...
		given := []*model.Document{
			fixModelDocumentForApp("1", bndlID()),
			fixModelDocumentForApp("2", bndlID()),
			fixModelDocumentForApp("3", bndlID()),
		}
		expected := []*document.Entity{
			fixEntityDocumentForApp("1", bndlID()),
			fixEntityDocumentForApp("2", bndlID()),
			fixEntityDocumentForApp("3", bndlID()),
		}
		...
		for i, givenModel := range given {
			...
			dbMock.ExpectExec(insertQuery).
				WithArgs(givenModel.ID, bndlID(), appID, repo.NewValidNullableString(""), givenModel.Title, givenModel.DisplayName, givenModel.Description, givenModel.Format, givenModel.Kind, givenModel.Data,
	...
	t.Run("DB Error", func(t *testing.T) {
		...
		dbMock.ExpectQuery(parentAccessQuery).WithArgs(givenTenant(), bndlID(), true).WillReturnRows(testdb.RowWhenObjectExist())
		dbMock.ExpectExec(insertQuery).
			WithArgs(expected[0].ID, bndlID(), appID, repo.NewValidNullableString(""), expected[0].Title, expected[0].DisplayName, expected[0].Description, expected[0].Format, expected[0].Kind, expected[0].Data,
		...
		dbMock.ExpectQuery(parentAccessQuery).WithArgs(givenTenant(), bndlID(), true).WillReturnRows(testdb.RowWhenObjectExist())
		dbMock.ExpectExec(insertQuery).
			WithArgs(expected[1].ID, bndlID(), appID, repo.NewValidNullableString(""), expected[1].Title, expected[1].DisplayName, expected[1].Description, expected[1].Format, expected[1].Kind, expected[1].Data,
...
func TestRepository_ListAllForBundle(t *testing.T) {
	...
	firstDocEntity := fixEntityDocumentForApp(firstDocID, onePageBundleID)
	firstDocModel := fixModelDocumentForApp(firstDocID, onePageBundleID)

	multiplePagesBundleID := "multiplePagesBundleID"

	secondDocID := "222222222-2222-2222-2222-222222222222"
	secondDocEntity := fixEntityDocumentForApp(secondDocID, multiplePagesBundleID)
	secondDocModel := fixModelDocumentForApp(secondDocID, multiplePagesBundleID)

	suite := testdb.RepoListPageableTestSuite{
		Name: "List Documents for multiple bundles with paging",
		SQLQueryDetails: []testdb.SQLQueryDetails{
			{
				Query: regexp.QuoteMeta(`(SELECT id, bundle_id, app_id, app_template_version_id, title, display_name, description, format, kind, data, ready, created_at, updated_at, deleted_at, error FROM public.documents WHERE (id IN (SELECT id FROM documents_tenants WHERE tenant_id = $1)) AND bundle_id = $2 ORDER BY bundle_id ASC, id ASC LIMIT $3 OFFSET $4)
												UNION
												(SELECT id, bundle_id, app_id, app_template_version_id, title, display_name, description, format, kind, data, ready, created_at, updated_at, deleted_at, error FROM public.documents WHERE (id IN (SELECT id FROM documents_tenants WHERE tenant_id = $5)) AND bundle_id = $6 ORDER BY bundle_id ASC, id ASC LIMIT $7 OFFSET $8)
												UNION
												(SELECT id, bundle_id, app_id, app_template_version_id, title, display_name, description, format, kind, data, ready, created_at, updated_at, deleted_at, error FROM public.documents WHERE (id IN (SELECT id FROM documents_tenants WHERE tenant_id = $9)) AND bundle_id = $10 ORDER BY bundle_id ASC, id ASC LIMIT $11 OFFSET $12)`),
				...
				ValidRowsProvider: func() []*sqlmock.Rows {
					return []*sqlmock.Rows{sqlmock.NewRows(columns).
						AddRow(firstDocID, firstDocEntity.BndlID, firstDocEntity.AppID, repo.NewValidNullableString(""), firstDocEntity.Title, firstDocEntity.DisplayName, firstDocEntity.Description, firstDocEntity.Format, firstDocEntity.Kind, firstDocEntity.Data, firstDocEntity.Ready, firstDocEntity.CreatedAt, firstDocEntity.UpdatedAt, firstDocEntity.DeletedAt, firstDocEntity.Error).
						AddRow(secondDocID, secondDocEntity.BndlID, secondDocEntity.AppID, repo.NewValidNullableString(""), secondDocEntity.Title, secondDocEntity.DisplayName, secondDocEntity.Description, secondDocEntity.Format, secondDocEntity.Kind, secondDocEntity.Data, secondDocEntity.Ready, secondDocEntity.CreatedAt, secondDocEntity.UpdatedAt, secondDocEntity.DeletedAt, secondDocEntity.Error),
					}
				},
			},
			{
...
func TestRepository_GetByID(t *testing.T) {
	docModel := fixModelDocumentForApp(givenID(), bndlID())
	docEntity := fixEntityDocumentForApp(givenID(), bndlID())

	suite := testdb.RepoGetTestSuite{
		Name: "Get Document",
		SQLQueryDetails: []testdb.SQLQueryDetails{
			{
				Query:    regexp.QuoteMeta(`SELECT id, bundle_id, app_id, app_template_version_id, title, display_name, description, format, kind, data, ready, created_at, updated_at, deleted_at, error FROM public.documents WHERE id = $1 AND (id IN (SELECT id FROM documents_tenants WHERE tenant_id = $2))`),
				Args:     []driver.Value{givenID(), givenTenant()},
				IsSelect: true,
				ValidRowsProvider: func() []*sqlmock.Rows {
					return []*sqlmock.Rows{
						sqlmock.NewRows(columns).
							AddRow(givenID(), docEntity.BndlID, docEntity.AppID, repo.NewValidNullableString(""), docEntity.Title, docEntity.DisplayName, docEntity.Description, docEntity.Format, docEntity.Kind, docEntity.Data, docEntity.Ready, docEntity.CreatedAt, docEntity.UpdatedAt, docEntity.DeletedAt, docEntity.Error),
					}
				},
				InvalidRowsProvider: func() []*sqlmock.Rows {
					return []*sqlmock.Rows{
...
func TestRepository_GetForBundle(t *testing.T) {
	...
	suite := testdb.RepoGetTestSuite{
		...
		SQLQueryDetails: []testdb.SQLQueryDetails{
			{
				Query:    regexp.QuoteMeta(`SELECT id, bundle_id, app_id, app_template_version_id, title, display_name, description, format, kind, data, ready, created_at, updated_at, deleted_at, error FROM public.documents WHERE id = $1 AND bundle_id = $2 AND (id IN (SELECT id FROM documents_tenants WHERE tenant_id = $3))`),
