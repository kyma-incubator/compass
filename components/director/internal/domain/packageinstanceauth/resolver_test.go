...
func TestResolver_RequestBundleInstanceAuthCreation(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			// when
			result, err := resolver.RequestBundleInstanceAuthCreation(context.Background(), testBundleID, *gqlRequestInput)

			// then
			assert.Equal(t, testCase.ExpectedResult, result)
			assert.Equal(t, testCase.ExpectedErr, err)

			mock.AssertExpectationsForObjects(t, persist, transact, svc, bundleSvc, converter)
		})
	}
}
...
func TestResolver_SetBundleInstanceAuth(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			// when
			result, err := resolver.SetBundleInstanceAuth(context.Background(), testAuthID, *gqlSetInput)
...
func TestResolver_RequestBundleInstanceAuthDeletion(t *testing.T) {
	...
	modelBundle := &model.Bundle{DefaultInstanceAuth: fixModelAuth()}
	...
	testCases := []struct {
	...
	}{
		{
			...
			ServiceFn: func() *automock.Service {
				svc := &automock.Service{}
				svc.On("Get", txtest.CtxWithDBMatcher(), id).Return(modelInstanceAuth, nil).Once()
				svc.On("RequestDeletion", txtest.CtxWithDBMatcher(), modelInstanceAuth, modelBundle.DefaultInstanceAuth).Return(true, nil).Once()
				return svc
			},
			BundleServiceFn: func() *automock.BundleService {
				svc := &automock.BundleService{}
				svc.On("GetByInstanceAuthID", txtest.CtxWithDBMatcher(), id).Return(modelBundle, nil).Once()
				return svc
			},
			ConverterFn: func() *automock.Converter {
				conv := &automock.Converter{}
		...
		},
		{
			...
			ServiceFn: func() *automock.Service {
				...
				svc.On("RequestDeletion", txtest.CtxWithDBMatcher(), modelInstanceAuth, modelBundle.DefaultInstanceAuth).Return(false, nil).Once()
				return svc
			},
			BundleServiceFn: func() *automock.BundleService {
				svc := &automock.BundleService{}
		...
		},
		{
			...
			ServiceFn: func() *automock.Service {
				svc := &automock.Service{}
				svc.On("Get", txtest.CtxWithDBMatcher(), id).Return(modelInstanceAuth, nil).Once()
				svc.On("RequestDeletion", txtest.CtxWithDBMatcher(), modelInstanceAuth, modelBundle.DefaultInstanceAuth).Return(false, testErr).Once()
				return svc
			},
			BundleServiceFn: func() *automock.BundleService {
				svc := &automock.BundleService{}
