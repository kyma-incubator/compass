...
func TestResolver_UnregisterApplication(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resolver := application.NewResolver(transact, svc, nil, oAuth20Svc, sysAuthSvc, nil, nil, nil, nil, eventingSvc, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
			resolver.SetConverter(converter)
...
func TestResolver_UnpairApplication(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			sysAuthSvc := testCase.SysAuthServiceFn()
			oAuth20Svc := testCase.OAuth20ServiceFn()
			resolver := application.NewResolver(transact, svc, nil, oAuth20Svc, sysAuthSvc, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
			resolver.SetConverter(converter)
...
func TestResolver_MergeApplications(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resolver := application.NewResolver(mockTransactioner, svc, nil, nil, nil, converter, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
...
func TestResolver_ApplicationsByLocalTenantID(t *testing.T) {
	// GIVEN
	localTenantID := "local-tenant-id"
	filter := []*graphql.LabelFilter{{Key: "key", Query: str.Ptr("query")}}
	first := 200
	cursor := graphql.PageCursor("cursor")

	modelApplications := fixApplicationPage([]*model.Application{fixModelApplication("foo", "tenant-foo", appName, "Bar")})
	gqlApplications := []*graphql.Application{fixGQLApplication("foo", appName, "Bar")}
	testErr := errors.New("Test error")
	...
	testCases := []struct {
		...
		Filter               []*graphql.LabelFilter
		First                *int
		Cursor               *graphql.PageCursor
		ExpectedApplications *graphql.ApplicationPage
		ExpectedErr          error
	}{
		{
			Name:          "Returns error when starting transaction failed",
			...
			ConverterFn: func() *automock.ApplicationConverter {
				conv := &automock.ApplicationConverter{}
				conv.AssertNotCalled(t, "MultipleToGraphQL")
				return conv
			},
			TransactionerFn: func(persistTx *persistenceautomock.PersistenceTx) *persistenceautomock.Transactioner {
				transact := &persistenceautomock.Transactioner{}
			...
			First:                &first,
			Cursor:               &cursor,
			ExpectedApplications: nil,
			ExpectedErr:          testErr,
		},
		{
			Name:          "Returns error when paging argument first is nil",
			PersistenceFn: txtest.PersistenceContextThatExpectsCommit,
			ServiceFn: func() *automock.ApplicationService {
				appSvc := &automock.ApplicationService{}
				return appSvc
			...
			Cursor:               &cursor,
			ExpectedApplications: nil,
			ExpectedErr:          apperrors.NewInvalidDataError("missing required parameter 'first'"),
		},
		{
			Name:            "ListByLocalTenantID returns error",
			PersistenceFn:   txtest.PersistenceContextThatExpectsCommit,
			TransactionerFn: txtest.TransactionerThatSucceeds,
			ServiceFn: func() *automock.ApplicationService {
				svc := &automock.ApplicationService{}
				svc.On("ListByLocalTenantID", contextParam, localTenantID, labelfilter.MultipleFromGraphQL(filter), first, string(cursor)).Return(nil, testErr).Once()
				return svc
			},
			ConverterFn: func() *automock.ApplicationConverter {
				conv := &automock.ApplicationConverter{}
		...
		},
		{
			...
			ServiceFn: func() *automock.ApplicationService {
				svc := &automock.ApplicationService{}
				svc.On("ListByLocalTenantID", contextParam, localTenantID, labelfilter.MultipleFromGraphQL(filter), first, string(cursor)).Return(nil, nil).Once()
				return svc
			},
			ConverterFn: func() *automock.ApplicationConverter {
				conv := &automock.ApplicationConverter{}
		...
		},
		{
			...
			ServiceFn: func() *automock.ApplicationService {
				svc := &automock.ApplicationService{}
				svc.On("ListByLocalTenantID", contextParam, localTenantID, labelfilter.MultipleFromGraphQL(filter), first, string(cursor)).Return(modelApplications, nil).Once()
				return svc
			},
			ConverterFn: func() *automock.ApplicationConverter {
				conv := &automock.ApplicationConverter{}
				conv.On("MultipleToGraphQL", modelApplications.Data).Return(gqlApplications).Once()
				return conv
			},
			LocalTenantID:        localTenantID,
			Filter:               filter,
			First:                &first,
			Cursor:               &cursor,
			ExpectedApplications: fixGQLApplicationPage(gqlApplications),
			ExpectedErr:          nil,
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			// WHEN
			result, err := resolver.ApplicationsByLocalTenantID(context.TODO(), testCase.LocalTenantID, testCase.Filter, testCase.First, testCase.Cursor)

			// then
			assert.Equal(t, testCase.ExpectedApplications, result)
			if testCase.ExpectedErr != nil {
...
func TestResolver_ApplicationsGlobal(t *testing.T) {
	// GIVEN
	modelApplication := fixModelApplication("foo", "tenant-foo", "Foo", "Lorem Ipsum")
	...
	gqlApplication := fixGQLApplication("foo", "Foo", "Lorem Ipsum")
	gqlTenant := fixGraphQLTenant("customer", tnt.TypeToStr(tnt.Customer))
	gqlApplicationsWithTenants := []*graphql.ApplicationWithTenants{
		{
			Application: gqlApplication,
			Tenants: []*graphql.Tenant{
				gqlTenant,
			},
		},
	}
	...
	testCases := []struct {
		...
		TransactionerFn   func(persistTx *persistenceautomock.PersistenceTx) *persistenceautomock.Transactioner
		ServiceFn         func() *automock.ApplicationService
		ConverterFn       func() *automock.ApplicationWithTenantsConverter
		InputLabelFilters []*graphql.LabelFilter
		ExpectedResult    *graphql.ApplicationWithTenantsPage
		ExpectedErr       error
	}{
		{
			Name:            "Success",
			...
			ServiceFn: func() *automock.ApplicationService {
				svc := &automock.ApplicationService{}
				svc.On("ListAllGlobalByFilter", contextParam, filter, first, after).Return(fixApplicationWithTenantsPage(modelApplicationsWithTenants), nil).Once()
				return svc
			},
			ConverterFn: func() *automock.ApplicationWithTenantsConverter {
				conv := &automock.ApplicationWithTenantsConverter{}
				conv.On("MultipleToGraphQL", modelApplicationsWithTenants).Return(gqlApplicationsWithTenants).Once()
				return conv
			},
			InputLabelFilters: gqlFilter,
			ExpectedResult:    fixGQLApplicationWithTenantsPage(gqlApplicationsWithTenants),
			ExpectedErr:       nil,
		},
		{
			...
			ServiceFn: func() *automock.ApplicationService {
				svc := &automock.ApplicationService{}
				svc.On("ListAllGlobalByFilter", contextParam, filter, first, after).Return(nil, testErr).Once()
				return svc
			},
			ConverterFn: func() *automock.ApplicationWithTenantsConverter {
				return &automock.ApplicationWithTenantsConverter{}
			},
			InputLabelFilters: gqlFilter,
			ExpectedResult:    nil,
			ExpectedErr:       testErr,
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resolver := application.NewResolver(transact, svc, nil, nil, nil, nil, converter, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")

			// WHEN
			result, err := resolver.ApplicationsGlobal(context.TODO(), testCase.InputLabelFilters, &first, &gqlAfter)
...
func TestResolver_Applications_DoubleAuthFlow(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resolver := application.NewResolver(transact, svc, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, appTemplateSvc, nil, nil, nil, selfRegisterDistinguishLabelKey, "sb-")
			resolver.SetConverter(converter)
...
func TestResolver_ApplicationsForRuntime(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resolver := application.NewResolver(transact, applicationSvc, nil, nil, nil, applicationConverter, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
...
func TestResolver_SetApplicationLabel(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resolver := application.NewResolver(transactioner, svc, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
			resolver.SetConverter(converter)
	...
	t.Run("Returns error when Label input validation failed", func(t *testing.T) {
		resolver := application.NewResolver(nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
...
func TestResolver_Webhooks(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resolver := application.NewResolver(mockTransactioner, nil, svc, nil, nil, nil, nil, converter, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
...
func TestResolver_Auths(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resolver := application.NewResolver(transact, nil, nil, nil, svc, nil, nil, nil, conv, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
...
func TestResolver_EventingConfiguration(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resolver := application.NewResolver(transact, nil, nil, nil, nil, converter, nil, nil, nil, eventingSvc, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
...
func TestResolver_Operations(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resolver := application.NewResolver(transact, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, operationSvc, operationConv, "", "")
...
func TestResolver_Bundles(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resolver := application.NewResolver(transact, nil, nil, nil, nil, nil, nil, nil, nil, nil, svc, converter, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
...
func TestResolver_APIDefinition(t *testing.T) {
	// GIVEN
	var modelBundleRef *model.BundleReference

	specID := "one"
	apiID := "foo"
	appID := "bar"
	modelAPI := fixModelAPIDef(apiID, appID, "name", "bar")
	modelSpec := fixModelAPISpecWithID(specID, apiID)
	gqlAPI := fixGQLAPIDefWithSpec(apiID, "name", "bar")
	app := fixGQLApplication("foo", "foo", "foo")
	testErr := errors.New("Test error")
	txGen := txtest.NewTransactionContextGenerator(testErr)
	...
	testCases := []struct {
		Name               string
		TransactionerFn    func() (*persistenceautomock.PersistenceTx, *persistenceautomock.Transactioner)
		APIDefinitionSvcFn func() *automock.APIDefinitionService
		SpecSvcFn          func() *automock.SpecService
		ConverterFn        func() *automock.APIDefinitionConverter
		InputID            string
		Application        *graphql.Application
	...
	}{
		{
			...
			APIDefinitionSvcFn: func() *automock.APIDefinitionService {
				...
				svc.On("GetForApplication", txtest.CtxWithDBMatcher(), appID, apiID).Return(modelAPI, nil).Once()
				return svc
			},
			SpecSvcFn: func() *automock.SpecService {
		...
		{
			Name:            "Success when API is not found",
			TransactionerFn: txGen.ThatSucceeds,
			APIDefinitionSvcFn: func() *automock.APIDefinitionService {
				svc := &automock.APIDefinitionService{}
				svc.On("GetForApplication", txtest.CtxWithDBMatcher(), appID, apiID).Return(nil, apperrors.NewNotFoundErrorWithType(resource.API)).Once()
				return svc
			},
			SpecSvcFn:   fixUnusedSpecService,
			ConverterFn: fixUnusedAPIDefinitionConverted,
			InputID:     appID,
			Application: app,
			Expected:    nil,
		},
		{
			Name:               "Error when transaction cannot be begun",
			TransactionerFn:    txGen.ThatFailsOnBegin,
			APIDefinitionSvcFn: fixUnusedAPIDefinitionService,
			SpecSvcFn:          fixUnusedSpecService,
			ConverterFn:        fixUnusedAPIDefinitionConverted,
			InputID:            appID,
			Application:        app,
		...
		{
			Name:            "Error when getting API for Application",
			TransactionerFn: txGen.ThatDoesntExpectCommit,
			APIDefinitionSvcFn: func() *automock.APIDefinitionService {
				svc := &automock.APIDefinitionService{}
				svc.On("GetForApplication", txtest.CtxWithDBMatcher(), appID, apiID).Return(nil, testErr).Once()
				return svc
			},
			SpecSvcFn:   fixUnusedSpecService,
			ConverterFn: fixUnusedAPIDefinitionConverted,
		...
		{
			Name:            "Error when getting Spec for API",
			TransactionerFn: txGen.ThatDoesntExpectCommit,
			APIDefinitionSvcFn: func() *automock.APIDefinitionService {
				svc := &automock.APIDefinitionService{}
				svc.On("GetForApplication", txtest.CtxWithDBMatcher(), appID, apiID).Return(modelAPI, nil).Once()
		...
		{
			Name:            "Error when converting to GraphQL object",
			TransactionerFn: txGen.ThatDoesntExpectCommit,
			APIDefinitionSvcFn: func() *automock.APIDefinitionService {
				svc := &automock.APIDefinitionService{}
				svc.On("GetForApplication", txtest.CtxWithDBMatcher(), appID, apiID).Return(modelAPI, nil).Once()
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			persist, transact := testCase.TransactionerFn()
			apiDefSvc := testCase.APIDefinitionSvcFn()
			specSvc := testCase.SpecSvcFn()
			converter := testCase.ConverterFn()
			defer mock.AssertExpectationsForObjects(t, persist, apiDefSvc, specSvc, converter)

			resolver := application.NewResolver(transact, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, specSvc, apiDefSvc, nil, nil, nil, nil, nil, converter, nil, nil, nil, nil, nil, "", "")

			// WHEN
			result, err := resolver.APIDefinition(context.TODO(), testCase.Application, testCase.InputID)
	...
	t.Run("Returns error when application is nil", func(t *testing.T) {
		resolver := application.NewResolver(nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
		// WHEN
		_, err := resolver.APIDefinition(context.TODO(), nil, "")
		// THEN
		require.Error(t, err)
		assert.EqualError(t, err, apperrors.NewInternalError("Application cannot be empty").Error())
	})
...
func TestResolver_EventDefinition(t *testing.T) {
	// GIVEN
	var modelBundleRef *model.BundleReference
	...
	eventID := "foo"
	appID := "bar"
	modelEvent := fixModelEventDef(eventID, appID, "name", "bar")
	modelSpec := fixModelEventSpecWithID(specID, eventID)
	gqlEvent := fixGQLEventDefWithSpec(eventID, "name", "bar")
	app := fixGQLApplication("foo", "foo", "foo")
	testErr := errors.New("Test error")
	txGen := txtest.NewTransactionContextGenerator(testErr)
	...
	testCases := []struct {
		Name                 string
		TransactionerFn      func() (*persistenceautomock.PersistenceTx, *persistenceautomock.Transactioner)
		EventDefinitionSvcFn func() *automock.EventDefinitionService
		SpecSvcFn            func() *automock.SpecService
		ConverterFn          func() *automock.EventDefinitionConverter
		InputID              string
		Application          *graphql.Application
	...
	}{
		{
			...
			EventDefinitionSvcFn: func() *automock.EventDefinitionService {
				...
				svc.On("GetForApplication", txtest.CtxWithDBMatcher(), appID, eventID).Return(modelEvent, nil).Once()
		...
		{
			Name:            "Success when Event is not found",
			TransactionerFn: txGen.ThatSucceeds,
			EventDefinitionSvcFn: func() *automock.EventDefinitionService {
				svc := &automock.EventDefinitionService{}
				svc.On("GetForApplication", txtest.CtxWithDBMatcher(), appID, eventID).Return(nil, apperrors.NewNotFoundErrorWithType(resource.API)).Once()
				return svc
			},
			SpecSvcFn:   fixUnusedSpecService,
			ConverterFn: fixUnusedEventDefinitionConverted,
			InputID:     appID,
			Application: app,
			Expected:    nil,
		},
		{
			Name:                 "Error when transaction cannot be begun",
			TransactionerFn:      txGen.ThatFailsOnBegin,
			EventDefinitionSvcFn: fixUnusedEventDefinitionService,
			SpecSvcFn:            fixUnusedSpecService,
			ConverterFn:          fixUnusedEventDefinitionConverted,
			InputID:              appID,
			Application:          app,
		...
		{
			Name:            "Error when getting Event for Application",
			TransactionerFn: txGen.ThatDoesntExpectCommit,
			EventDefinitionSvcFn: func() *automock.EventDefinitionService {
				svc := &automock.EventDefinitionService{}
				svc.On("GetForApplication", txtest.CtxWithDBMatcher(), appID, eventID).Return(nil, testErr).Once()
		...
		{
			Name:            "Error when getting Spec for Event",
			TransactionerFn: txGen.ThatDoesntExpectCommit,
			EventDefinitionSvcFn: func() *automock.EventDefinitionService {
				svc := &automock.EventDefinitionService{}
				svc.On("GetForApplication", txtest.CtxWithDBMatcher(), appID, eventID).Return(modelEvent, nil).Once()
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			persist, transact := testCase.TransactionerFn()
			eventDefSvc := testCase.EventDefinitionSvcFn()
			specSvc := testCase.SpecSvcFn()
			converter := testCase.ConverterFn()
			defer mock.AssertExpectationsForObjects(t, persist, eventDefSvc, specSvc, converter)

			resolver := application.NewResolver(transact, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, specSvc, nil, eventDefSvc, nil, nil, nil, nil, nil, converter, nil, nil, nil, nil, "", "")

			// WHEN
			result, err := resolver.EventDefinition(context.TODO(), testCase.Application, testCase.InputID)
	...
	t.Run("Returns error when application is nil", func(t *testing.T) {
		resolver := application.NewResolver(nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
		// WHEN
		_, err := resolver.EventDefinition(context.TODO(), nil, "")
		// THEN
		require.Error(t, err)
		assert.EqualError(t, err, apperrors.NewInternalError("Application cannot be empty").Error())
	})
...
func TestResolver_ApplicationTemplate(t *testing.T) {
	...
	id := "foo"
	appTemplateID := "12345678-ae7e-4d1a-8027-520a96d5319d"
	appTemplateName := "app-template"

	gqlApp := fixGQLApplication(id, "name", "desc")
	gqlAppWithAppTemplate := fixGQLApplicationWithAppTemplate(id, "name", "desc", appTemplateID)
	modelAppTemplate := fixModelApplicationTemplate(appTemplateID, appTemplateName)
	gqlAppTemplate := fixGQLApplicationTemplate(appTemplateID, appTemplateName)
	...
	testCases := []struct {
		...
		Application     *graphql.Application
		ExpectedResult  *graphql.ApplicationTemplate
		ExpectedErr     error
	}{
		{
			Name:            "Success",
			...
			ServiceFn: func() *automock.ApplicationTemplateService {
				svc := &automock.ApplicationTemplateService{}
				svc.On("Get", contextParam, appTemplateID).Return(modelAppTemplate, nil).Once()
				return svc
			},
			ConverterFn: func() *automock.ApplicationTemplateConverter {
				conv := &automock.ApplicationTemplateConverter{}
				conv.On("ToGraphQL", modelAppTemplate).Return(gqlAppTemplate, nil).Once()
				return conv
			},
			Application:    gqlAppWithAppTemplate,
			ExpectedResult: gqlAppTemplate,
			ExpectedErr:    nil,
		},
		{
		...
		{
			Name:            "Returns error when app template is not found",
			PersistenceFn:   txtest.PersistenceContextThatDoesntExpectCommit,
			TransactionerFn: txtest.TransactionerThatDoesARollback,
			ServiceFn: func() *automock.ApplicationTemplateService {
				svc := &automock.ApplicationTemplateService{}
				svc.On("Get", contextParam, appTemplateID).Return(nil, testErr).Once()
				return svc
			},
			ConverterFn: func() *automock.ApplicationTemplateConverter {
				conv := &automock.ApplicationTemplateConverter{}
			...
			Application:    gqlAppWithAppTemplate,
			ExpectedResult: nil,
			ExpectedErr:    errors.Wrapf(testErr, "no app template found with id %s", appTemplateID),
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resolver := application.NewResolver(transact, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, svc, converter, nil, nil, "", "")
			// WHEN
			result, err := resolver.ApplicationTemplate(context.TODO(), testCase.Application)
...
func TestResolver_IntegrationDependencies(t *testing.T) {
	// GIVEN
	testErr := errors.New("test error")

	firstAppID := "appID"
	secondAppID := "appID2"
	firstIntegrationDependencyID := "integrationDependencyID"
	secondIntegrationDependencyID := "integrationDependencyID2"
	firstAspectID := "aspectID"
	secondAspectID := "aspectID2"
	appIDs := []string{firstAppID, secondAppID}

	integrationDependencyFirstApp := fixModelIntegrationDependency(firstIntegrationDependencyID, firstAppID, "Foo", "Lorem Ipsum")
	integrationDependencySecondApp := fixModelIntegrationDependency(secondIntegrationDependencyID, secondAppID, "Foo", "Lorem Ipsum")

	integrationDependenciesFirstApp := []*model.IntegrationDependency{integrationDependencyFirstApp}
	integrationDependenciesSecondApp := []*model.IntegrationDependency{integrationDependencySecondApp}

	gqlIntegrationDependencyFirstApp := fixGQLIntegrationDependency(firstIntegrationDependencyID, "Foo", "Lorem Ipsum")
	gqlIntegrationDependencySecondApp := fixGQLIntegrationDependency(secondIntegrationDependencyID, "Foo", "Lorem Ipsum")

	gqlIntegrationDependenciesFirstApp := []*graphql.IntegrationDependency{gqlIntegrationDependencyFirstApp}
	gqlIntegrationDependenciesSecondApp := []*graphql.IntegrationDependency{gqlIntegrationDependencySecondApp}

	integrationDependencyPageFirstApp := fixIntegrationDependencyPage(integrationDependenciesFirstApp)
	integrationDependencyPageSecondApp := fixIntegrationDependencyPage(integrationDependenciesSecondApp)
	integrationDependencyPages := []*model.IntegrationDependencyPage{integrationDependencyPageFirstApp, integrationDependencyPageSecondApp}

	gqlIntegrationDependencyPageFirstApp := fixGQLIntegrationDependencyPage(gqlIntegrationDependenciesFirstApp)
	gqlIntegrationDependencyPageSecondApp := fixGQLIntegrationDependencyPage(gqlIntegrationDependenciesSecondApp)
	gqlIntegrationDependencyPages := []*graphql.IntegrationDependencyPage{gqlIntegrationDependencyPageFirstApp, gqlIntegrationDependencyPageSecondApp}

	numberOfIntDepsInFirstApplication := 1
	numberOfIntDepsInSecondApplication := 1
	intDepFirstApplicationAspects := fixModelIntegrationDependencyAspect(firstAspectID, firstAppID, firstIntegrationDependencyID)
	intDepSecondApplicationAspects := fixModelIntegrationDependencyAspect(secondAspectID, secondAppID, secondIntegrationDependencyID)
	aspectsFirstIntegrationDependency := []*model.Aspect{intDepFirstApplicationAspects}
	aspectsSecondIntegrationDependency := []*model.Aspect{intDepSecondApplicationAspects}
	aspects := []*model.Aspect{intDepFirstApplicationAspects, intDepSecondApplicationAspects}
	intDepFirstApplicationAspectEventResources := fixModelAspectEventResource(firstAppID, firstAspectID)
	intDepSecondApplicationAspectEventResources := fixModelAspectEventResource(secondAppID, secondAspectID)
	aspectEventResourcesFirstAspect := map[string][]*model.AspectEventResource{firstAspectID: {intDepFirstApplicationAspectEventResources}}
	aspectEventResourcesSecondAspect := map[string][]*model.AspectEventResource{secondAspectID: {intDepSecondApplicationAspectEventResources}}
	aspectEventResources := []*model.AspectEventResource{intDepFirstApplicationAspectEventResources, intDepSecondApplicationAspectEventResources}
	totalCounts := map[string]int{firstAppID: numberOfIntDepsInFirstApplication, secondAppID: numberOfIntDepsInSecondApplication}
	...
	testCases := []struct {
		Name                         string
		TransactionerFn              func() (*persistenceautomock.PersistenceTx, *persistenceautomock.Transactioner)
		IntDepServiceFn              func() *automock.IntegrationDependencyService
		IntDepConverterFn            func() *automock.IntegrationDependencyConverter
		AspectServiceFn              func() *automock.AspectService
		AspectEventResourceServiceFn func() *automock.AspectEventResourceService
		ExpectedResult               []*graphql.IntegrationDependencyPage
		ExpectedErr                  []error
	}{
		{
			Name:            "Success",
			...
			IntDepServiceFn: func() *automock.IntegrationDependencyService {
				...
				svc.On("ListByApplicationIDs", txtest.CtxWithDBMatcher(), appIDs, first, after).Return(integrationDependencyPages, nil).Once()
				return svc
			},
			IntDepConverterFn: func() *automock.IntegrationDependencyConverter {
				conv := &automock.IntegrationDependencyConverter{}
				conv.On("ToGraphQL", integrationDependencyFirstApp, aspectsFirstIntegrationDependency, aspectEventResourcesFirstAspect).Return(gqlIntegrationDependencyFirstApp, nil).Once()
				conv.On("ToGraphQL", integrationDependencySecondApp, aspectsSecondIntegrationDependency, aspectEventResourcesSecondAspect).Return(gqlIntegrationDependencySecondApp, nil).Once()
				return conv
			},
			AspectServiceFn: func() *automock.AspectService {
				svc := &automock.AspectService{}
				svc.On("ListByApplicationIDs", txtest.CtxWithDBMatcher(), appIDs, first, after).Return(aspects, totalCounts, nil).Once()
				return svc
			},
			AspectEventResourceServiceFn: func() *automock.AspectEventResourceService {
				svc := &automock.AspectEventResourceService{}
				svc.On("ListByApplicationIDs", txtest.CtxWithDBMatcher(), appIDs, first, after).Return(aspectEventResources, totalCounts, nil).Once()
				return svc
			},
			ExpectedResult: gqlIntegrationDependencyPages,
			ExpectedErr:    nil,
		},
		{
			Name:            "Returns error when transaction begin failed",
		...
		},
		{
			...
			AspectServiceFn: func() *automock.AspectService {
				svc := &automock.AspectService{}
				svc.On("ListByApplicationIDs", txtest.CtxWithDBMatcher(), appIDs, first, after).Return(nil, nil, testErr).Once()
				return svc
			},
			AspectEventResourceServiceFn: func() *automock.AspectEventResourceService {
				svc := &automock.AspectEventResourceService{}
		...
		},
		{
			...
			IntDepConverterFn: func() *automock.IntegrationDependencyConverter {
				conv := &automock.IntegrationDependencyConverter{}
				conv.On("ToGraphQL", integrationDependencyFirstApp, aspectsFirstIntegrationDependency, aspectEventResourcesFirstAspect).Return(nil, testErr).Once()
				return conv
			},
			AspectServiceFn: func() *automock.AspectService {
				svc := &automock.AspectService{}
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			// GIVEN
			persist, transact := testCase.TransactionerFn()
			svc := testCase.IntDepServiceFn()
			converter := testCase.IntDepConverterFn()
			aspectService := testCase.AspectServiceFn()
			aspectEventResourceSvc := testCase.AspectEventResourceServiceFn()

			firstAppParams := dataloader.ParamIntegrationDependency{ID: firstAppID, Ctx: context.TODO(), First: &first, After: &gqlAfter}
			secondAppParams := dataloader.ParamIntegrationDependency{ID: secondAppID, Ctx: context.TODO(), First: &first, After: &gqlAfter}
			keys := []dataloader.ParamIntegrationDependency{firstAppParams, secondAppParams}
			resolver := application.NewResolver(transact, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, svc, converter, aspectService, aspectEventResourceSvc, nil, nil, nil, nil, nil, nil, "", "")

			// WHEN
			result, err := resolver.IntegrationDependenciesDataLoader(keys)
			...
			mock.AssertExpectationsForObjects(t, persist, transact, svc, converter, aspectService, aspectEventResourceSvc)
		})
	}
	...
	t.Run("Returns error when there are no Applications", func(t *testing.T) {
		resolver := application.NewResolver(nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
		// WHEN
		_, err := resolver.IntegrationDependenciesDataLoader([]dataloader.ParamIntegrationDependency{})
		// THEN
		require.Error(t, err[0])
		assert.EqualError(t, err[0], apperrors.NewInternalError("No Applications found").Error())
	})
	...
	t.Run("Returns error when start cursor is nil", func(t *testing.T) {
		firstAppParams := dataloader.ParamIntegrationDependency{ID: firstAppID, Ctx: context.TODO(), First: nil, After: &gqlAfter}
		keys := []dataloader.ParamIntegrationDependency{firstAppParams}

		resolver := application.NewResolver(nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", "")
		// WHEN
		_, err := resolver.IntegrationDependenciesDataLoader(keys)
		// THEN
		require.Error(t, err[0])
		assert.EqualError(t, err[0], apperrors.NewInvalidDataError("missing required parameter 'first'").Error())
	})
