...
func TestResolver_ApplicationsForRuntime(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			//WHEN
			result, err := resolver.ApplicationsForRuntime(context.TODO(), testCase.InputRuntimeID.String(), &first, &gqlAfter)
...
func TestResolver_Documents(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resolver := application.NewResolver(transact, nil, nil, nil, svc, nil, nil, converter, nil, nil, nil)
