...
func TestRepository_Unpair(t *testing.T) {
	t.Run("Success when operation mode is set to sync and no operation in the context", func(t *testing.T) {
		//given
		appModel := fixDetailedModelApplication(t, givenID(), givenTenant(), "Test app", "Test app description")
		...
		// when
		err := repo.Unpair(ctx, appModel)

		// then
		require.NoError(t, err)
	})

	t.Run("Success when operation mode is set to async and there is an operation in the context", func(t *testing.T) {
		//given
		ctx := context.Background()
		ctx = operation.SaveModeToContext(ctx, graphql.OperationModeAsync)
