...
// GetForPackage provides a mock function with given fields: ctx, tenantID, id, packageID
func (_m *BundleRepository) GetForPackage(ctx context.Context, tenantID string, id string, packageID string) (*model.Bundle, error) {
	ret := _m.Called(ctx, tenantID, id, packageID)

	var r0 *model.Bundle
	if rf, ok := ret.Get(0).(func(context.Context, string, string, string) *model.Bundle); ok {
		r0 = rf(ctx, tenantID, id, packageID)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*model.Bundle)
		}
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, string, string, string) error); ok {
		r1 = rf(ctx, tenantID, id, packageID)
	} else {
		r1 = ret.Error(1)
	}
