...
// ListByScenariosNoPaging lists all runtimes that are in any of the given scenarios
func (r *pgRepository) ListByScenariosNoPaging(ctx context.Context, tenant string, scenarios []string) ([]*model.Runtime, error) {
	tenantUUID, err := uuid.Parse(tenant)
	if err != nil {
		return nil, apperrors.NewInvalidDataError("tenantID is not UUID")
	}
	...
	for _, rtEnt := range entities {
		m := r.conv.FromEntity(&rtEnt)
