...
func (r *Resolver) UnregisterIntegrationSystem(ctx context.Context, id string) (*graphql.IntegrationSystem, error) {
	...
	auths, err := r.sysAuthSvc.ListForObject(ctx, systemauth.IntegrationSystemReference, intSys.ID)
	if err != nil {
		return nil, err
	}
...
func (r *Resolver) Auths(ctx context.Context, obj *graphql.IntegrationSystem) ([]*graphql.IntSysSystemAuth, error) {
	...
	sysAuths, err := r.sysAuthSvc.ListForObject(ctx, systemauth.IntegrationSystemReference, obj.ID)
	if err != nil {
		return nil, err
	}
