...
type statusService interface {
	UpdateWithConstraints(ctx context.Context, notificationResponse *statusreport.NotificationStatusReport, fa *model.FormationAssignment, operation model.FormationOperation) error
	SetAssignmentToErrorStateWithConstraints(ctx context.Context, notificationResponse *statusreport.NotificationStatusReport, assignment *model.FormationAssignment, errorMessage string, errorCode AssignmentErrorCode, state model.FormationAssignmentState, operation model.FormationOperation) error
	DeleteWithConstraints(ctx context.Context, id string, notificationResponse *statusreport.NotificationStatusReport) error
}
...
func (s *service) processFormationAssignmentsWithReverseNotification(ctx context.Context, mappingPair *AssignmentMappingPairWithOperation, depth int, isReverseProcessed *bool) error {
	...
	statusReport := newNotificationStatusReportFromWebhookResponse(response)

	if response.Error != nil && *response.Error != "" {
		err = s.statusService.SetAssignmentToErrorStateWithConstraints(ctx, statusReport, assignment, *response.Error, ClientError, model.CreateErrorAssignmentState, mappingPair.Operation)
		if err != nil {
	...
	if err = s.statusService.UpdateWithConstraints(ctx, statusReport, assignment, mappingPair.Operation); err != nil {
...
func (s *service) CleanupFormationAssignment(ctx context.Context, mappingPair *AssignmentMappingPairWithOperation) (bool, error) {
	...
	if response.Error != nil && *response.Error != "" {
		if err = s.statusService.SetAssignmentToErrorStateWithConstraints(ctx, statusReport, assignment, *response.Error, ClientError, model.DeleteErrorAssignmentState, mappingPair.Operation); err != nil {
	...
	if response.State != nil && *response.State == string(model.ReadyAssignmentState) {
		if err = s.statusService.DeleteWithConstraints(ctx, assignment.ID, statusReport); err != nil {
			if apperrors.IsNotFoundError(err) {
				log.C(ctx).Infof("Assignment with ID %q has already been deleted", assignment.ID)
				return false, nil
			}
	...
	if response.State != nil && *response.State == string(model.DeleteErrorAssignmentState) {
		if err = s.statusService.SetAssignmentToErrorStateWithConstraints(ctx, statusReport, assignment, "", ClientError, model.DeleteErrorAssignmentState, mappingPair.Operation); err != nil {
			if apperrors.IsNotFoundError(err) {
				log.C(ctx).Infof("Assignment with ID %q has already been deleted", assignment.ID)
				return false, nil
			}
	...
	if response.State != nil && *response.State == string(model.ConfigPendingAssignmentState) {
