...
type repository struct {
	...
	formationTemplateUpdaterGlobal repo.UpdaterGlobal
...
func NewRepository(conv EntityConverter) *repository {
	return &repository{
		...
		formationTemplateUpdaterGlobal: repo.NewUpdaterGlobal(resource.Webhook, tableName, updatableColumns, []string{"id", "formation_template_id"}),
...
func (r *repository) Update(ctx context.Context, tenant string, item *model.Webhook) error {
	...
	if item.ObjectType.GetResourceType() == resource.FormationTemplateWebhook && tenant == "" {
		return r.formationTemplateUpdaterGlobal.UpdateSingleGlobal(ctx, entity)
	}
