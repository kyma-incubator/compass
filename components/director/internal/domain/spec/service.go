...
// GetDataHashByID todo
func (s *service) GetDataHashByID(ctx context.Context, id string, objectType model.SpecReferenceObjectType) (*string, error){
	tnt, err := tenant.LoadFromContext(ctx)
	if err != nil {
		return nil, err
	}

	return s.repo.GetDataHashByID(ctx, tnt, id, objectType)
}
