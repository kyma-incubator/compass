...
func TestPgRepository_Update(t *testing.T) {
	...
	suite := testdb.RepoUpdateTestSuite{
		...
		SQLQueryDetails: []testdb.SQLQueryDetails{
			{
				Query:         regexp.QuoteMeta(`UPDATE public.vendors SET title = ?, labels = ?, partners = ?, tags = ?, documentation_labels = ? WHERE id = ? AND (id IN (SELECT id FROM vendors_tenants WHERE tenant_id = ? AND owner = true))`),
...
func TestPgRepository_UpdateGlobal(t *testing.T) {
	...
	suite := testdb.RepoUpdateTestSuite{
		...
		SQLQueryDetails: []testdb.SQLQueryDetails{
			{
				Query:         regexp.QuoteMeta(`UPDATE public.vendors SET title = ?, labels = ?, partners = ?, tags = ?, documentation_labels = ? WHERE id = ?`),
