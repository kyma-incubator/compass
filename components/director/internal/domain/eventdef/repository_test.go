...
func TestPgRepository_ListForBundle(t *testing.T) {
	...
	selectQuery := fmt.Sprintf(`SELECT (.+) FROM "public"."event_api_definitions"
		WHERE tenant_id = \$1 AND id IN \(SELECT (.+) FROM public\.bundle_references WHERE tenant_id = \$2 AND bundle_id = \$3 AND event_def_id IS NOT NULL\) 
		ORDER BY id LIMIT %d OFFSET %d`, ExpectedLimit, ExpectedOffset)

	countQuery := `SELECT COUNT\(\*\) FROM "public"."event_api_definitions"
		WHERE tenant_id = \$1 AND id IN \(SELECT (.+) FROM public\.bundle_references WHERE tenant_id = \$2 AND bundle_id = \$3 AND event_def_id IS NOT NULL\)`
...
func TestPgRepository_ListAllForBundle(t *testing.T) {
	...
	t.Run("success when there are no more pages", func(t *testing.T) {
		...
		pgRepository := event.NewRepository(convMock)
		// WHEN
		modelEventDefs, err := pgRepository.ListAllForBundle(ctx, tenantID, bundleIDs, bundleRefs, totalCounts, inputPageSize, inputCursor)
		//THEN
		require.NoError(t, err)
	...
	t.Run("success when there is next page and it can be traversed", func(t *testing.T) {
		...
		modelEventDefsSecondPage, err := pgRepository.ListAllForBundle(ctx, tenantID, bundleIDs, bundleRefsSecondPage, totalCounts, inputPageSize, endCursor)
		//THEN
		require.NoError(t, err)
...
func TestPgRepository_Update(t *testing.T) {
	updateQuery := regexp.QuoteMeta(`UPDATE "public"."event_api_definitions" SET package_id = ?, name = ?, description = ?, group_name = ?, ord_id = ?,
		...
		version_for_removal = ?, ready = ?, created_at = ?, updated_at = ?, deleted_at = ?, error = ?, extensible = ?, successors = ? WHERE tenant_id = ? AND id = ?`)
	...
	t.Run("success", func(t *testing.T) {
		...
		sqlMock.ExpectExec(updateQuery).
			WithArgs(entity.PackageID, entity.Name, entity.Description, entity.GroupName, entity.OrdID, entity.ShortDescription, entity.SystemInstanceAware, entity.ChangeLogEntries, entity.Links,
				...
				entity.Disabled, entity.PartOfProducts, entity.LineOfBusiness, entity.Industry, entity.Version.Value, entity.Version.Deprecated, entity.Version.DeprecatedSince, entity.Version.ForRemoval, entity.Ready, entity.CreatedAt, entity.UpdatedAt, entity.DeletedAt, entity.Error, entity.Extensible, entity.Successors, tenantID, entity.ID).
			WillReturnResult(sqlmock.NewResult(-1, 1))
...
func TestPgRepository_DeleteAllByBundleID(t *testing.T) {
	sqlxDB, sqlMock := testdb.MockDatabase(t)
	ctx := persistence.SaveToContext(context.TODO(), sqlxDB)
	deleteQuery := `DELETE FROM "public"."event_api_definitions"
		WHERE tenant_id = \$1 AND id IN \(SELECT (.+) FROM public\.bundle_references WHERE tenant_id = \$2 AND bundle_id = \$3 AND event_def_id IS NOT NULL\)`
