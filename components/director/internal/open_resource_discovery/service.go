...
func NewServiceConfig(maxParallelSpecificationProcessors int, credentialExchangeStrategyTenantMappings map[string]processors.CredentialExchangeStrategyTenantMapping) ServiceConfig {
...
type Service struct {
	...
	bundleSvc             processors.BundleService
	bundleProcessor       processors.BundleProcessor
...
func NewAggregatorService(config ServiceConfig, metricsCfg MetricsConfig, transact persistence.Transactioner, appSvc ApplicationService, webhookSvc processors.WebhookService, bundleSvc processors.BundleService, bundleReferenceSvc BundleReferenceService, apiSvc APIService, eventSvc EventService, specSvc SpecService, fetchReqSvc FetchRequestService, packageSvc processors.PackageService, packageProcessor processors.PackageProcessor, productSvc processors.ProductService, productProcessor processors.ProductProcessor, vendorSvc processors.VendorService, vendorProcessor processors.VendorProcessor, tombstoneProcessor TombstoneProcessor, tenantSvc TenantService, globalRegistrySvc GlobalRegistryService, client Client, appTemplateVersionSvc ApplicationTemplateVersionService, appTemplateSvc ApplicationTemplateService, labelService LabelService, ordWebhookMapping []application.ORDWebhookMapping, opSvc operationsmanager.OperationService, bundleProcessor processors.BundleProcessor) *Service {
	return &Service{
		config:                config,
...
func (s *Service) processDocuments(ctx context.Context, resource Resource, webhookBaseURL, webhookBaseProxyURL string, ordRequestObject requestobject.OpenResourceDiscoveryWebhookRequestObject, documents Documents, globalResourcesOrdIDs map[string]bool, validationErrors *error) error {
	...
	for _, doc := range documents {
		...
		bundlesFromDB, err := s.bundleProcessor.Process(ctx, resourceToAggregate.Type, resourceToAggregate.ID, doc.ConsumptionBundles, resourceHashes)
		if err != nil {
			return err
		}
