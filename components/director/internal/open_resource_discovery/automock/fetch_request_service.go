...
func (_m *FetchRequestService) FetchSpec(ctx context.Context, fr *model.FetchRequest, headers http.Header) (*string, *model.FetchRequestStatus) {
	ret := _m.Called(ctx, fr, headers)

	var r0 *string
	if rf, ok := ret.Get(0).(func(context.Context, *model.FetchRequest, http.Header) *string); ok {
	...
	if rf, ok := ret.Get(1).(func(context.Context, *model.FetchRequest, http.Header) *model.FetchRequestStatus); ok {
