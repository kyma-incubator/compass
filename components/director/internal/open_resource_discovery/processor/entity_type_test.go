	...
	testCases := []struct {
	...
	}{
		{
			...
			EntityTypeSvcFn: func() *automock.EntityTypeService {
				...
				entityTypeSvc.On("Update", txtest.CtxWithDBMatcher(), resource.Application, appID, packageID, *entityTypeInputs[0], uint64ResourceHash).Return(nil).Once()
		...
		},
		{
			...
			EntityTypeSvcFn: func() *automock.EntityTypeService {
				...
				entityTypeSvc.On("Update", txtest.CtxWithDBMatcher(), resource.ApplicationTemplateVersion, appTemplateVersionID, packageID, *entityTypeInputs[0], uint64ResourceHash).Return(nil).Once()
				return entityTypeSvc
			},
		...
		},
		{
			...
			EntityTypeSvcFn: func() *automock.EntityTypeService {
				entityTypeSvc := &automock.EntityTypeService{}
				entityTypeSvc.On("ListByApplicationID", txtest.CtxWithDBMatcher(), appID).Return(entityTypeModels, nil).Once()
				entityTypeSvc.On("Update", txtest.CtxWithDBMatcher(), resource.Application, appID, packageID, *entityTypeInputs[0], uint64ResourceHash).Return(testErr).Once()
				return entityTypeSvc
			},
			InputResource:       resource.Application,
			InputResourceID:     appID,
