...
func (c *Client) sendRequestWithRetry(req *http.Request) (*http.Response, error) {
	...
	err := retry.Do(func() error {
		...
		retry.Attempts(c.apiConfig.RetryAttempts),
		retry.Delay(c.apiConfig.RetryInterval),
		retry.LastErrorOnly(true),
		retry.RetryIf(func(err error) bool {
			return strings.Contains(err.Error(), "connection refused") ||
				strings.Contains(err.Error(), "connection reset by peer") ||
				strings.Contains(err.Error(), fmt.Sprintf("status code %d", http.StatusInternalServerError))
		}))
