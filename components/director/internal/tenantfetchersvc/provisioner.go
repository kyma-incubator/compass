...
func (p *provisioner) tenantsFromRequest(request TenantSubscriptionRequest) []model.BusinessTenantMappingInput {
	...
	accountTenant := p.newAccountTenant(request.AccountTenantID, customerID, request.Subdomain, request.Region, licenseType)
	...
	if len(request.SubaccountTenantID) > 0 {
		tenants = append(tenants, p.newSubaccountTenant(request.SubaccountTenantID, accountID, request.Subdomain, request.Region, licenseType))
	}
	return tenants
}

func (p *provisioner) newCustomerTenant(tenantID string, licenseType *string) model.BusinessTenantMappingInput {
	return p.newTenant(tenantEntity.TrimCustomerIDLeadingZeros(tenantID), "", "", "", autogeneratedTenantProvider, licenseType, tenantEntity.Customer)
}

func (p *provisioner) newAccountTenant(tenantID, parent, subdomain, region string, licenseType *string) model.BusinessTenantMappingInput {
	return p.newTenant(tenantID, tenantEntity.TrimCustomerIDLeadingZeros(parent), subdomain, region, p.tenantProvider, licenseType, tenantEntity.Account)
}

func (p *provisioner) newSubaccountTenant(tenantID, parent, subdomain, region string, licenseType *string) model.BusinessTenantMappingInput {
	return p.newTenant(tenantID, parent, subdomain, region, p.tenantProvider, licenseType, tenantEntity.Subaccount)
}

func (p *provisioner) newTenant(tenantID, parent, subdomain, region, provider string, licenseType *string, tenantType tenantEntity.Type) model.BusinessTenantMappingInput {
