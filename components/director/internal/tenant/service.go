...
const (
	...
	LabelTenantDatabaseError           = "Something went wrong while labeling tenant"
...
func NewService(tenant TenantRepository, labelRepository LabelRepository, transact persistence.Transactioner, uidService UIDService, config Config) *service {
	return &service{
		tenantRepo: tenant,
		transact:   transact,
		labelRepo:  labelRepository,
...
func (s *service) Create(writer http.ResponseWriter, request *http.Request) {
	...
	for _, t := range tenantsToCreate {
		...
		if _, err := s.tenantRepo.GetByExternalID(ctx, t.TenantId); err != nil {
			...
			subdomainLabel := model.NewLabelForTenant(t, subdomainLabelKey, t.Subdomain)
			if err := s.labelRepo.Upsert(ctx, subdomainLabel); err != nil {
				logger.WithError(err).Errorf("while labeling tenant with subdomain: %v", err)
				http.Error(writer, LabelTenantDatabaseError, http.StatusInternalServerError)
				return
			}
...
func (s *service) getParentTenantInternalID(ctx context.Context, tenant model.TenantModel) (string, error) {
	...
	parentTenant, err := s.tenantRepo.GetByExternalID(ctx, tenant.ParentExternalId)
	if err == nil {
	...
	if err = s.tenantRepo.Create(ctx, autogeneratedParentTenant); err != nil {
