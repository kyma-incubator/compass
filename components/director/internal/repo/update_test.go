	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			t.Run("success for BundleInstanceAuth", func(t *testing.T) {
				...
				mock.ExpectExec(regexp.QuoteMeta(fmt.Sprintf("UPDATE %s SET name = ?, description = ?%s WHERE id = ? AND (id IN (SELECT %s FROM %s WHERE %s = '%s' AND %s = true) OR owner_id = ?)", biaTableName, test.AdditionalCondition, repo.M2MResourceIDColumn, m2mTable, repo.M2MTenantIDColumn, tenantID, repo.M2MOwnerColumn))).
					WithArgs(biaName, biaDescription, biaID, tenantID).WillReturnResult(sqlmock.NewResult(0, 1))
				// WHEN
				err := test.Method(updater)(ctx, resourceType, tenantID, fixBIA)
				// THEN
				require.NoError(t, err)
			})
