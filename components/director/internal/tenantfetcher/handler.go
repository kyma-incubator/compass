...
func NewTenantsHTTPHandler(tenantSvc TenantService, transact persistence.Transactioner, uidService UIDService, config HandlerConfig) *handler {
	return &handler{
		tenantSvc:  tenantSvc,
		transact:   transact,
		uidService: uidService,
...
func (h *handler) Create(writer http.ResponseWriter, request *http.Request) {
	...
	if err := h.labelTenantWithSubdomain(ctx, accountTenant.ID, subdomain); err != nil {
...
func (h *handler) labelTenantWithSubdomain(ctx context.Context, tenantID, subdomain string) error {
	...
	return h.tenantSvc.SetLabel(ctx, labelIn)
}
