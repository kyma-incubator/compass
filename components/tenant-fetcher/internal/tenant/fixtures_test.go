...
const (
	testID                           = "foo"
	createQuery                      = "INSERT INTO public.business_tenant_mappings ( id, external_name, external_tenant, subdomain, parent, type, provider_name, status ) VALUES ( ?, ?, ?, ?, ?, ?, ? )"
	getByExternalIDQueryFormat       = "SELECT id, external_name, external_tenant, subdomain, parent, type, provider_name, status FROM public.business_tenant_mappings WHERE external_tenant='%s'"
	updateQueryFormat                = "UPDATE public.business_tenant_mappings SET external_name = ?, subdomain = ?, parent = ?, type = ?, provider_name = ?, status = ? WHERE external_tenant='%s'"
...
func fixSQLRows(rows []sqlRow) *sqlmock.Rows {
	...
	for _, row := range rows {
		out.AddRow(row.id, row.name, row.externalTenant, row.subdomain, row.parent, row.tenantType, row.provider, row.status)
	}
	return out
}
