...
func (s *service) Create(writer http.ResponseWriter, request *http.Request) {
	...
	for _, t := range tenantsToCreate {
		if len(t.ParentInternalId) <= 0 && len(t.ParentExternalId) > 0 {
			parentEntity, err := s.repository.GetByExternalID(ctx, t.ParentExternalId)
			if err != nil {
			...
			t.ParentInternalId = parentEntity.ID
		}
