...
//			cli := tc.GraphQLClientFn()
//			cliProvider := fixProviderMock(httpReq, cli)
//			gqlRequester := &svcautomock.GraphQLRequester{}
//			gqlRequester.On("GetApplicationRequest", id).Return(expectedGQLReq, nil).Once()
//			converter := tc.ConverterFn()
//
//			defer mock.AssertExpectationsForObjects(t, cli, cliProvider, gqlRequester, converter)
//
//			w := httptest.NewRecorder()
//
//			handler := NewHandler(cliProvider, converter, nil, gqlRequester, logger)
//
//			// when
//
...
//			cliProvider := fixProviderMock(httpReq, cli)
//			gqlBuilder := &svcautomock.GraphQLRequester{}
