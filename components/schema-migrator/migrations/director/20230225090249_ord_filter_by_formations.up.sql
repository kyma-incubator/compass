...
-- drop views that rely on 'apps_subaccounts' view so that we can drop and change it
...
DROP VIEW IF EXISTS apps_subaccounts;

-- enhance the apps_subaccounts view to return not only the ID of the application and the tenant ID (subaccount) in formation with it but also the formation ID

CREATE VIEW apps_subaccounts(id, tenant_id, formation_id)
AS
...
CREATE OR REPLACE VIEW tenants_apis
            (tenant_id, formation_id, id, app_id, name, description, group_name, default_auth, version_value,
             ...
             system_instance_aware, policy_level, custom_policy_level, api_protocol, tags, countries, links, api_resource_links, release_status,
...
CREATE OR REPLACE VIEW tenants_events
            (tenant_id, formation_id, id, app_id, name, description, group_name, version_value,
             version_deprecated, version_deprecated_since, version_for_removal, ord_id, short_description,
             system_instance_aware, policy_level, custom_policy_level, changelog_entries, links, tags, countries, release_status, sunset_date, labels,
...
CREATE OR REPLACE VIEW tenants_vendors
            (tenant_id, formation_id, ord_id, app_id, title, labels, partners, id, documentation_labels)
AS
SELECT DISTINCT t_apps.tenant_id,
                t_apps.formation_id,
                v.ord_id,
