...
-- create formations index on tenant_id column
CREATE INDEX formations_tenant_id ON formations(tenant_id);

-- create new view which will serve the following purpose:

-- we need to calculate the formation_id of the formation/s based on the scenario label of all Applications that are in formation/s
-- this is needed for the App Template subscription flow in ORD Service
-- up to this change, the formation_id was missing in that flow because it is extracted from a JOIN between labels and ASAs but for the AppTemplates there are no ASAs involved and we have to calculate the formation_ids directly from the scenario label of the Apps that are in formation/s

CREATE VIEW apps_formations_id(app_id, formation_id) AS
SELECT l.app_id AS app_id, f.id AS formation_id
FROM labels l
         JOIN formations f ON f.tenant_id = l.tenant_id
WHERE app_id is not null and l.key = 'scenarios' and l.value ?| array[f.name];

-- recreate views, enhancing them by using information from the new view above

-- in addition, a new default tenant value is introduced 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb' that will be used in the views
-- it was needed because we changed the filtering approaches of the ORD Service: up to this point, it could have filtered based on tenant_id or tenant_id + formation_id
-- however, this is no longer suitable for the future scenarios, so now we allow the ORD Service to filter based on tenant_id or formation_id

-- since we no longer need tenant_id when we want to filter by formation_id, the apps_subaccounts usage in the views was removed in favour of the new apps_formations_id
