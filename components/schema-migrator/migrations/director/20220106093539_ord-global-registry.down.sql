...
ALTER TABLE vendors ALTER COLUMN app_id SET NOT NULL;
...
ALTER TABLE vendors ADD CONSTRAINT vendor_ord_id_unique UNIQUE (app_id, ord_id);

ALTER TABLE products DROP CONSTRAINT product_ord_id_unique;
ALTER TABLE products ADD CONSTRAINT product_ord_id_unique UNIQUE (app_id, ord_id);

ALTER TABLE products ADD CONSTRAINT products_vendor_fk FOREIGN KEY (app_id, vendor) REFERENCES vendors(app_id, ord_id);
ALTER TABLE packages ADD CONSTRAINT packages_vendor_fk FOREIGN KEY (app_id, vendor) REFERENCES vendors(app_id, ord_id);
...
FROM api_definitions,
     jsonb_array_elements_text(api_definitions.part_of_products) AS elements
         JOIN products p ON elements.value = p.ord_id WHERE p.app_id = api_definitions.app_id;
...
FROM event_api_definitions,
     jsonb_array_elements_text(event_api_definitions.part_of_products) AS elements
         JOIN products p ON elements.value = p.ord_id WHERE p.app_id = event_api_definitions.app_id;
