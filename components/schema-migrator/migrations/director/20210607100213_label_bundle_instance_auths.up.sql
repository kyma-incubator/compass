...
--- BEGIN LABELING EXISTING BUNDLE INSTANCE AUTHS
...
DO $$
    ...
    BEGIN
        ...
        CREATE VIEW bundle_instance_auths_scenarios AS SELECT bundle_instance_auths.id, bundles.tenant_id, app_id, runtime_id
        ...
        FOR rec IN SELECT "id", tenant_id, app_id, runtime_id FROM bundle_instance_auths_scenarios LOOP
                ...
                IF NOT EXISTS (SELECT * FROM labels WHERE bundle_instance_auth_id=rec.id) AND common_scenarios IS NOT NULL THEN
