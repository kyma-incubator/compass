...
CREATE OR REPLACE VIEW tenants_apis
            (tenant_id, id, app_id, name, description, group_name, default_auth, version_value, version_deprecated,
...
FROM api_definitions apis
         JOIN (SELECT a1.id,
               ...
               WHERE l.app_id IS NOT NULL
                 AND l.key::text = 'scenarios'::text) t_apps ON apis.app_id = t_apps.id
               ...
               FROM labels l
                        JOIN automatic_scenario_assignments asa
                             ON asa.tenant_id = l.tenant_id AND l.value ? asa.scenario::text AND
                                asa.selector_key::text = 'global_subaccount_id'::text
               WHERE l.app_id IS NOT NULL
                 AND l.key::text = 'scenarios'::text) t_apps ON apps.id = t_apps.id;

DROP VIEW IF EXISTS tenants_bundles;

CREATE OR REPLACE VIEW tenants_bundles
            (tenant_id, id, app_id, name, description, instance_auth_request_json_schema, default_instance_auth, ord_id,
               ...
               FROM labels l
                        JOIN automatic_scenario_assignments asa
                             ON asa.tenant_id = l.tenant_id AND l.value ? asa.scenario::text AND
                                asa.selector_key::text = 'global_subaccount_id'::text
               WHERE l.app_id IS NOT NULL
                 AND l.key::text = 'scenarios'::text) t_apps ON b.app_id = t_apps.id;

DROP VIEW IF EXISTS tenants_events;

CREATE OR REPLACE VIEW tenants_events
            (tenant_id, id, app_id, name, description, group_name, version_value, version_deprecated,
             version_deprecated_since, version_for_removal, ord_id, short_description, system_instance_aware,
