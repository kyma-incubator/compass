...
FROM (SELECT *
    ...
    AND value IS NOT NULL) l1 -- Get the subaccount for each runtime
...
FROM api_definitions apis
         JOIN (SELECT a1.id,
               ...
               UNION ALL
               SELECT a_s.id, (SELECT id FROM business_tenant_mappings WHERE external_tenant = a_s.tenant_id::text)::text, cpr.provider_tenant::text AS provider_tenant_id
