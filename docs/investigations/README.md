# Investigations

## Overview

This directory contains the following investigations:

- [Storage - Requirements](./storage/requirements.md)
- [Storage - S3](./storage/s3/README.md)
- [Storage - S3 - Parquet](./storage/s3/parquet.md)
- [Storage - Managed DB](./storage/managed-db/managed-dbs.md)
- [Storage - PostgreSQL Evaluation](./storage/postgres/postgres.md)
- [Storage - SQL Toolbox](./storage/sql-toolbox/sql-toolbox.md)
- [GraphQL - File Upload](./graphql-file-upload/file-upload-with-graphql.md)
- [GraphQL - Mutation for updating complex types](./graphql-complex-updates/graphql-complex-updates.md)
- [GraphQL - Secure Resolvers](./secure-resolvers/secure-resolvers.md)
- [Documentation - Generating static page for our schema](./api-documentation/generate-api-documentation.md)
