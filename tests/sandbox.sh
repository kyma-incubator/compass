...
source "$CURRENT_PATH"/../installation/scripts/utils.sh

COMPONENT=$1
echo "Component name: $COMPONENT"
kubectl_k3d_kyma get testdefinition compass-e2e-$COMPONENT -n kyma-system -o json > td_duplicate.json
...
kubectl_k3d_kyma apply -f td_duplicate.json
rm td_duplicate.json

cat <<EOF | sed "s/PLACEHOLDER/$COMPONENT-local/" | kubectl_k3d_kyma -n kyma-system apply -f -
...
containerStatus=$(kubectl_k3d_kyma get pods -n kyma-system oct-tp-compass-e2e-tests-compass-e2e-$COMPONENT-local-0  -ojsonpath="{.status.containerStatuses[?(@.name=='$COMPONENT-tests')].state.running}")
...
kubectl_k3d_kyma exec -n kyma-system oct-tp-compass-e2e-tests-compass-e2e-$COMPONENT-local-0 -c $COMPONENT-tests -- apk add go