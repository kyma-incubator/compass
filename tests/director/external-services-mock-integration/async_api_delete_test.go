...
func TestAsyncAPIDeleteApplicationWithAppTemplateWebhook(t *testing.T) {
	...
	appTemplateInputGQL, err := tc.Graphqlizer.ApplicationTemplateInputToGQL(appTemplateInput)
	require.NoError(t, err)
	registerTemplateRequest := fixRegisterApplicationTemplateRequest(appTemplateInputGQL)
	...
	appFromTemplateInputGQL, err := tc.Graphqlizer.ApplicationFromTemplateInputToGQL(appFromAppTemplateInput)
	require.NoError(t, err)
	registerAppRequest := fixRegisterApplicationFromTemplateRequest(appFromTemplateInputGQL)
	app := graphql.ApplicationExt{}
...
func TestAsyncAPIDeleteApplicationPrioritizationWithBothAppTemplateAndAppWebhook(t *testing.T) {
	...
	err = tc.RunOperationWithCustomTenant(ctx, dexGraphQLClient, testConfig.DefaultTenant, registerAppWebhookRequest, &webhookResult)
	require.NoError(t, err)
...
func triggerAsyncDeletion(t *testing.T, ctx context.Context, app graphql.ApplicationExt, appNearCreationTime time.Time, expectedWebhookID string, gqlClient *gcli.Client) {
	...
	unregisterAsyncApplicationInTenant(t, ctx, gqlClient, app.ID, testConfig.DefaultTenant)
