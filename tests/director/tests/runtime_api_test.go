...
func TestSelfRegMoreThanOneProviderRuntime(t *testing.T) {
	...
	require.Error(t, err)
	require.Equal(t, err.Error(), fmt.Sprintf("Cannot have more than one runtime with labels %q: %q and %q: %q", tenantfetcher.RegionKey, conf.SubscriptionConfig.SelfRegRegion, conf.SubscriptionConfig.SelfRegDistinguishLabelKey, conf.SubscriptionConfig.SelfRegDistinguishLabelValue))
