...
func TestOperationSchedule(t *testing.T) {
	...
	t.Log("Creating integration system")
	intSys, err := fixtures.RegisterIntegrationSystem(t, ctx, certSecuredGraphQLClient, tenantId, "int-sys-ops-schedule")
	defer fixtures.CleanupIntegrationSystem(t, ctx, certSecuredGraphQLClient, tenantId, intSys)
	require.NoError(t, err)
	require.NotEmpty(t, intSys.ID)
	...
	op := graphql.Operation{}
	err = testctx.Tc.RunOperationWithoutTenant(ctx, intSystemOAuthGraphQLClient, scheduleOperationRequest, &op)
	require.NoError(t, err)
	require.NotNil(t, op)
	require.NotEmpty(t, op.ID)
	require.Equal(t, graphql.ScheduledOperationTypeOrdAggregation, op.OperationType)
