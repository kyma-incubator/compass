...
func TestRuntimeContextsFormationProcessingFromASA(stdT *testing.T) {
	...
	t.Run("Create Automatic Scenario Assignment BEFORE runtime creation", func(t *testing.T) {
		...
		t.Run("Register kyma runtime and validate scenarios label", func(t *testing.T) {
		...
		t.Run("Register provider runtime and validate scenarios label", func(t *testing.T) {
			...
			// Validate scenarios label exists for provider runtime
			t.Logf("Assert scenarios label after assigning tenant with ID: %q to formation", subscriptionConsumerSubaccountID)
			checkRuntimeContextFormationLabelsForRuntime(t, ctx, subscriptionConsumerAccountID, providerRuntime.ID, "scenarios", []string{providerFormationName})
			...
			// Validate scenarios label DOES NOT exists for kyma runtime
			t.Log("Assert kyma runtime has NOT scenarios label")
			checkRuntimeFormationLabelIsMissing(t, ctx, subscriptionConsumerAccountID, kymaRuntime.ID)
		})
	})
	...
	t.Run("FINAL", func(t *testing.T) {
		...
		// Validate kyma runtime scenarios labels
		t.Logf("Assert kyma runtime HAS kyma scenarios label after assigning tenant with ID: %q to formation", subscriptionConsumerSubaccountID)
		checkRuntimeFormationLabelsExists(t, ctx, subscriptionConsumerAccountID, kymaRuntime.ID, "scenarios", []string{kymaFormationName})

		t.Logf("Assert kyma runtime HAS NOT provider scenarios label after assigning tenant with ID: %q to formation", subscriptionConsumerSubaccountID)
		checkRuntimeFormationLabelIsMissing(t, ctx, subscriptionConsumerAccountID, providerRuntime.ID)

		// Validate provider runtime scenarios labels
		t.Logf("Assert provider runtime HAS provider scenarios label after assigning tenant with ID: %q to formation", subscriptionConsumerSubaccountID)
		checkRuntimeContextFormationLabelsForRuntime(t, ctx, subscriptionConsumerAccountID, providerRuntime.ID, "scenarios", []string{providerFormationName})

		t.Logf("Assert provider runtime HAS NOT kyma scenarios label after assigning tenant with ID: %q to formation", subscriptionConsumerSubaccountID)
		checkRuntimeContextFormationLabelsForRuntimeIsMissing(t, ctx, subscriptionConsumerAccountID, kymaRuntime.ID, "scenarios")
	})
}
